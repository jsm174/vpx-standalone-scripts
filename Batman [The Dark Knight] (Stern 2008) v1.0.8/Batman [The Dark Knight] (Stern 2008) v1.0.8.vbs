...
'1.08 Retro27 - Reworked DMD decal for VR Cabinet, Added VR Auto Select
...
'///////////////////////-----Batman Model-----///////////////////////
Const BMModel = 0  ' 0 - Removed Batman Model, 1 - Add Batman Model
...
Sub FlashFlasher(nr)
	...
	Select Case nr
		Case 1 : UpdateMaterial "meshflash187",0,0,0,0,0,0,ObjLevel(nr)^3,RGB(247,247,247),0,0,False,True,0,0,0,0
		Case 2 : UpdateMaterial "meshflash189",0,0,0,0,0,0,ObjLevel(nr)^3,RGB(247,247,247),0,0,False,True,0,0,0,0
	End Select
...
sub flash189(level)
	ObjTargetLevel(2) = level/255 : FlasherFlash2_Timer
end sub
...
Sub FlashFadeLevel(Level,TargetLevel,TimerEnabled)
	TimerEnabled = True
	if round(TargetLevel,1) > round(Level,1) Then
		Level = Level + 0.3
		if Level > 1 then Level = 1
	Elseif round(TargetLevel,1) < round(Level,1) Then
		Level = Level * 0.85 - 0.01
		if Level < 0 then Level = 0
	Else
		Level = round(TargetLevel,1)
		TimerEnabled = False
	end if
End Sub

flasher_scareinsert.visible = false
flasher_backwall.visible = false
flasher_joker.visible = false
...
Dim Sol19lvl, Sol19target, Sol19Enable

sub flash179(level)
	Sol19target = level/255 : flasher_scareinsert_timer
end sub

sub flasher_scareinsert_timer
	if Not flasher_scareinsert.TimerEnabled then				' This is here to detect the first call to this timer
		flasher_scareinsert.visible = true
		flasher_scareinsert.TimerEnabled = true				' enabling this timer
	end if

	FlashFadeLevel sol19lvl,Sol19target,Sol19Enable

	flasher_scareinsert.opacity = 5630 * sol19lvl^3
	psc.blenddisablelighting = 82 * sol19lvl

	if Not Sol19Enable Then
		flasher_scareinsert.visible = false
		flasher_scareinsert.TimerEnabled = false				' disabling this timer
	end if
end sub
...
Dim Sol21lvl, Sol21target, Sol21Enable

sub flash181(level)
	Sol21target = level/255 : flasher_backwall_timer
end sub

sub flasher_backwall_timer
	if Not flasher_backwall.TimerEnabled then				' This is here to detect the first call to this timer
		flasher_backwall.visible = true
		flasher_backwall.TimerEnabled = true				' enabling this timer
	end if

	FlashFadeLevel Sol21lvl,Sol21target,Sol21Enable

	flasher_backwall.opacity = 230 * Sol21lvl^3

	if Not Sol21Enable Then
		flasher_backwall.visible = false
		flasher_backwall.TimerEnabled = false				' disabling this timer
	end if
end sub
...
Dim Sol22lvl, Sol22target, Sol22Enable

sub flash182(level)
	Sol22target = level/255 : flasher_joker_timer
end sub

sub flasher_joker_timer
	if Not flasher_joker.TimerEnabled then				' This is here to detect the first call to this timer
		flasher_joker.visible = true
		flasher_joker.TimerEnabled = true				' enabling this timer
	end if

	FlashFadeLevel Sol22lvl,Sol22target,Sol22Enable

	flasher_joker.opacity = 130 * sol22lvl^3
	flasher_joker.visible = true
	flasher_joker.opacity = 130 * sol22lvl
	bakerybumps.blenddisablelighting = 1.25 * sol22lvl+1
	bakerybumps001.blenddisablelighting = 1.25 * sol22lvl+1
	batmobile_spot.blenddisablelighting = 1.05 * sol22lvl+1
	batmobile_spot001.blenddisablelighting = 2 * sol22lvl+1
	cilin.blenddisablelighting = .35 * sol22lvl
	cilout.blenddisablelighting = .35 * sol22lvl
	UpdateMaterial "meshflash182",0,0,0,0,0,0,sol22lvl^3,RGB(247,247,247),0,0,False,True,0,0,0,0

	if Not Sol22Enable Then
		flasher_joker.visible = false
		flasher_joker.TimerEnabled = false				' disabling this timer
	end if
end sub
...
Dim Sol23lvl, Sol23target, Sol23Enable

sub flash183(level)
	Sol23target = level/255 : flasher_scarecrow_timer
end sub

sub flasher_scarecrow_timer
	if Not flasher_scarecrow.TimerEnabled then				' This is here to detect the first call to this timer
		flasher_scarecrow.visible = true
		flasher_scarecrow.TimerEnabled = true				' enabling this timer
	end if

	FlashFadeLevel Sol23lvl,Sol23target,Sol23Enable

	flasher_scarecrow.opacity = 50 * sol23lvl^3
	bakery2rsling003.blenddisablelighting = .05 * sol23lvl+1
	bakery2rsling.blenddisablelighting = .05 * sol23lvl+1
	bakerybumps.blenddisablelighting = .05 * sol23lvl+1
	bakerybumps001.blenddisablelighting = .05 * sol23lvl+1
	bakery004.blenddisablelighting = .1 * sol23lvl+1
	bakery4.blenddisablelighting = .1 * sol23lvl+1
	UpdateMaterial "meshflash183",0,0,0,0,0,0,sol23lvl^3,RGB(247,247,247),0,0,False,True,0,0,0,0

	if Not Sol23Enable Then
		flasher_scarecrow.visible = false
		flasher_scarecrow.TimerEnabled = false				' disabling this timer
	end if
end sub
...
Dim Sol25lvl, Sol25target, Sol25Enable

sub flash185(level)
	Sol25target = level/255 : flasher_bumpers_timer
end sub

sub flasher_bumpers_timer
	if Not flasher_bumpers.TimerEnabled then				' This is here to detect the first call to this timer
		flasher_bumpers.visible = true
		flasher_bumpers.TimerEnabled = true				' enabling this timer
	end if

	FlashFadeLevel Sol25lvl,Sol25target,Sol25Enable

	flasher_bumpers.opacity = 10 * sol25lvl^3
	cilin.blenddisablelighting = .15 * sol25lvl
	cilout.blenddisablelighting = .15 * sol25lvl
	UpdateMaterial "meshflash185",0,0,0,0,0,0,sol25lvl^3,RGB(247,247,247),0,0,False,True,0,0,0,0

	if Not Sol25Enable Then
		flasher_bumpers.visible = false
		flasher_bumpers.TimerEnabled = false				' disabling this timer
	end if
end sub
...
Dim Sol32lvl, Sol32target, Sol32Enable

sub flash192(level)
	Sol32target = level/255 : flasher_batmobile_timer
end sub

sub flasher_batmobile_timer
	if Not flasher_batmobile.TimerEnabled then				' This is here to detect the first call to this timer
		flasher_batmobile.visible = true
		flasher_batmobile.TimerEnabled = true				' enabling this timer
	end if

	FlashFadeLevel Sol32lvl,Sol32target,Sol32Enable

	flasher_batmobile.opacity = 30 * sol32lvl^3
	bakery2lsling003.blenddisablelighting = 4 * sol32lvl+1
	bakery2lsling.blenddisablelighting = 4 * sol32lvl+1
	UpdateMaterial "meshflash192",0,0,0,0,0,0,sol32lvl^3,RGB(247,247,247),0,0,False,True,0,0,0,0

	if Not Sol32Enable Then
		flasher_batmobile.visible = false
		flasher_batmobile.TimerEnabled = false				' disabling this timer
	end if
end sub
...
'			bulbBW.BlendDisableLighting = 0
			PinCab_Backglass.Blenddisablelighting=0
		End If
	End Select
End Sub
...
'	Lampz.Callback(30) = "DisableLighting p30, 25,"
	...
	Lampz.Callback(111) = "DisableLighting backwall_fils, 25,"
	Lampz.Callback(111) = "DisableLighting bulbBW, 5,"
...
Sub GIupdates(ByVal aLvl)	'GI update odds and ends go here
	...
	If aLvl>0 then
		...
		flasherbase1.blenddisablelighting=0.3
...
'*******************************************
	...
	Dim VRRoom
	If RenderingMode = 2 Then VRRoom = 1
	DIM VRThings
	If VRRoom > 0 Then
		ScoreText.visible = 0
		If VRRoom = 1 Then
...
If BMModel = 1 then
		Cab_bmmodel.visible=1
Else
		Cab_bmmodel.visible=0
end If
