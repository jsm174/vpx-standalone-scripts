...
'   actually the layout it is based on the prototype Keith Elwin made called Archer
'                          VISUAL PINBALL X.7
...
Const bFreePlay = False 'freeplay (True) or coins (False)
...
Sub EndOfBall()
    ...
    If NOT Tilted Then
        ...
        DMD CL(0, "ILUSSIA LETTERS X " & IlussiaBonusCount), CL(1, FormatScore(AwardPoints)), "", eNone, eNone, eNone, 1000, True, ""
        ...
        TotalBonus = TotalBonus + AwardPoints
        DMD CL(0, "PYRAMIDS X " & PyramidBonusCount), CL(1, FormatScore(AwardPoints)), "", eNone, eNone, eNone, 1000, True, ""

        'Mancha Xanandra completed
        AwardPoints = ManchaXBonusCount * 250000
        TotalBonus = TotalBonus + AwardPoints
        DMD CL(0, "MANCHA XANANDRA " & ManchaXBonusCount), CL(1, FormatScore(AwardPoints)), "", eNone, eNone, eNone, 1000, True, ""
...
Sub AwardSkillshot()
    ...
    'show dmd animation
    DMD CL(0, FormatScore(SkillshotValue(CurrentPlayer))), CL(1, ("SKILLSHOT")), "", eBlinkFast, eNone, eNone, 1500, True, "fanfare2"
    DOF 127, DOFPulse
...
Sub AwardSuperSkillshot()
    ResetSkillShotTimer_Timer
    'show dmd animation
    DMD CL(0, FormatScore(SuperSkillshotValue(CurrentPlayer))), CL(1, ("SUPER SKILLSHOT")), "", eBlinkFast, eNone, eNone, 1500, True, "fanfare3"
    DOF 127, DOFPulse
...
Sub LampTimer_Timer()
    Flashm 1, Flasher001big
    Lampm 1, LFlasher001
    Flash 1, flasher001
    Lampm 2, LFlasher002
    Flashm 2, flasher002a
    Flashm 2, Flasher002big
    Flash 2, flasher002
    Lampm 3, LFlasher003
    Flash 3, flasher003
    Lampm 4, LFlasher004
    ...
    Flashm 4, Flasher004big
    Flash 4, flasher004
    Lampm 5, LFlasher005
    Flashm 5, flasher005a
    Flashm 5, flasher005b
    Flashm 5, Flasher005big
    Flash 5, flasher005
    Flash 6, flasher006
    Lampm 7, LFlasher006
    Lampm 7, LFlasher007
...
Sub InstantInfo
    ...
    DMD CL(0, "FINISTERRA JACKPOT"), CL(1, FinisterraSJPValue(Currentplayer)), "", eNone, eNone, eNone, 800, False, ""
...
Sub Trigger008_Hit 'Left ramp done
    ...
    Select Case Mode(CurrentPlayer, 0)
        ...
        Case 4
            If Light054.State Then
                DMD " CELTIC LAND COMBO", CL(1, FormatScore(CelticValue) & " X " & CelticMultiplier), "", eNone, eBlink, eNone, 1500, True, ""
                light054.State = 0
                light053.State = 2
            ...
            Else
                DMD "CELTIC LAND RAMPSHOT", CL(1, FormatScore(CelticValue)), "", eNone, eBlink, eNone, 1500, True, ""
    ...
    Select Case Mode(CurrentPlayer, 0)
        ...
        Case 4
            If Light053.State Then
                ...
                CelticCombo.Enabled = 1
            Else
                DMD "CELTIC LAND RAMPSHOT", CL(1, FormatScore(CelticValue)), "", eNone, eBlink, eNone, 1500, True, "vo_nicerampshot"
                AddScore CelticValue
                CelticValue = CelticValue + 150000
            End If
        Case 5
...
Sub ArbolesTimer1_Timer 'countdown JP value
    ...
    DMD "  SHOOT THE WITCH", CL(1, FormatScore(ArbolesJPValue)), "", eNone, eNone, eNone, 200, True, ""
    ...
    Select Case FolkCount
        ...
        Case 6:
            If LastSwitchHit = "bruja_target" Then
                DMD "_", CL(1, "CAPTURED 1 GHOST"), "", eNone, eBlink, eNone, 1500, True, ""
                Light057.State = 0
                Light054.State = 2
                Light053.State = 2
            Else
        ...
        Case 8:
            If LastSwitchHit = "bruja_target" Then
                DMD "_", CL(1, "CAPTURED 2 GHOSTS"), "", eNone, eBlink, eNone, 1500, True, ""
                Light057.State = 0
            ...
            Else
                ...
                Light057.State = 0
            End If
        Case 9:DMD "_", CL(1, "SHOOT WITCH TARGET"), "", eNone, eBlink, eNone, 1500, True, ""
            FolkTimer.Enabled = 0
            For X = 0 to 7
                aLightArrows(x).State = 0
            Next
...
Sub StartIlussiaMUltiball
    StopBRUJALetter
    DMD CL(0, "ILUSSIA"), CL(1, "MULTIBALL"), "", eBlink, eBlink, eNone, 1500, True, "vo_multiball2"
    AddMultiball 1
    post001.IsDropped = 0:PlaySoundAt "fx_SolenoidOn", Trigger010
    post002.IsDropped = 0
    vpmtimer.addtimer 2500, "PlaySoundAt""fx_SolenoidOff"", Trigger010: post001.IsDropped = 1:post002.IsDropped = 1 '"
...
Sub CheckLockedBalls 'updates also the lights
    Select Case BallsLocked(CurrentPlayer)
        ...
        Case 1:light007.State = 1:light006.State = 0:light070.State = 0
            LightEffect 1
            DMD CL(0, "WITCHS NEST"), CL(1, "BALL 1 LOCKED"), "", eNone, eBlink, eNone, 1500, True, "vo_ball1locked"
            vpmtimer.addtimer 2500, "Exit_WitchNest '"
        Case 2:light007.State = 1:light006.State = 1:light070.State = 0
            LightEffect 1
            DMD CL(0, "WITCHS NEST"), CL(1, "BALL 2 LOCKED"), "", eNone, eBlink, eNone, 1500, True, "vo_ball2locked"
            vpmtimer.addtimer 2500, "Exit_WitchNest '"
        Case 3:
            If Mode(CurrentPlayer, 0) = 10 Then
                DMD CL(0, "WITCHS NEST"), CL(1, "BALL 3 LOCKED"), "", eNone, eBlink, eNone, 1500, True, "vo_ball3locked"
                light007.State = 2:light006.State = 2:light070.State = 2
                LightEffect 1
                light058.State = 2
                bLockActive(CurrentPlayer) = False
                bLockActive(CurrentPlayer) = False
                DMD CL(0, "WITCH"), CL(1, "MULTIBALL IS READY"), "", eNone, eBlink, eNone, 5000, True, ""
...
Sub StartWitchMultiball
    ...
    ' flash show
    DMD CL(0, "WITCH"), CL(1, "MULTIBALL"), "", eNone, eBlink, eNone, 1500, True, "vo_multiball3"
    Exit_WitchNest
    post001.IsDropped = 0:PlaySoundAt "fx_SolenoidOn", Trigger010
    post002.IsDropped = 0
...
Sub AwardSPJackpot
    If bSPJActivated Then
        ...
        DMD CL(0, FormatScore(tmp)), "SUPR PYRAMID JACKPOT", "", eNone, eBlinkFast, eNone, 1500, True, "vo_superjackpot2"
        DOF 126, DOFPulse
        Addscore tmp
        LightEffect 2
        StopSuperPyramid
...
Sub StartFinisterra
    StopBRUJALetter
    DMD CL(0, "FINISTERRA"), "     MULTIBALL", "", eNone, eBlink, eNone, 1500, True, "vo_multiball"
    light045.State = 0
    light024.State = 0
    light025.State = 0
    light026.State = 0
...
Sub AwardFinisterraSJP
    ...
    DMD CL(0, FormatScore(tmp)), "FINISTERRA SJACKPOT", "", eNone, eBlinkFast, eNone, 1500, True, "vo_superjackpot"
    bFinisterraSJPActivated = False
    light045.State = 0
    light024.State = 0
    light025.State = 0
...
Sub AwardTombTreasure
    ...
    Select Case TombTreasureCount
        ...
        Case 7:
            DMD CL(0, "TOMB AWARD"), CL(1, "SUPER PYRAMID 5X SPJ"), "", eNone, eBlink, eNone, 1500, True, ""
