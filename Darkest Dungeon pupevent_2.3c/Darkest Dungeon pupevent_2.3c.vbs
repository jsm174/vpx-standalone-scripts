...
'         DarkestDungeon v2.3 - PuP and PUPDMD
' ****************************************************************
' August 2024
' Table Version: 2.3
' PuP and PUPDMD mod: V2.2c
...
' Additional Testers			passion4pins, loadedweapon,pegula0420, 10mmsocket, GeradG



'2.3
' Fixed Cball Jackpot Glitch
' Changed Scorbit to new Codes
' Added Tweak Menu for Options
' Added Scorbit Timeline
...
'         USER OPTIONS
'//////////////////////////////////////////////////////////////////////
Const ScorbitActive				= 0 	' Is Scorbit Active	, Turn on by changing this value to 1
...
Const FlipperBounce 			= 0.6		' Recommended values .5-.88 (.88 default nFozzy, .6 Default for this Table)
'/////////////////////////////////////////////////////////////////////
'*************************************************************
' VR Room Auto-Detect
'*************************************************************
Dim VR_Obj, VRMode, VRRoomChoice

	VRRoomChoice 				= 1			' 1 = Cathedral Room, 2 = Sphere Dungeon
...
' *********************************************************************
Dim DefaultBall:DefaultBall			= 4 ' Determines which ball is loaded as the default ball
									' 0 - Silver Glow
									' 1 - Purple Glow
									' 2 - Green Glow
									' 3 - Blue Glow
...
Dim VolumeDial' = 0.8
Dim BallRollVolume' = 0.5 			'Level of ball rolling volume. Value between 0 and 1
Dim RampRollVolume' = 0.5 			'Level of ramp rolling volume. Value between 0 and 1
...
Dim WoodRamps':WoodRamps = True
' Radiant MODE - Adds 5 seconds to heartbeat timers, changes outlane posts position 
Dim Difficulty':Difficulty 				= 1    ' 1 - Radiant , 2 - Darkest , 3 - Stygian (easy,med,hard)
Dim StagedFlippers':StagedFlippers 			= 0         '0 = Normal  1 = Staged Flippers
Dim MaxDarkness':MaxDarkness 				= 95 ' determines how dark the playfield can get (higher number equals darker : 80-99 is recommended range, Default is 95
Dim DefaultDarkness':DefaultDarkness			= 5  ' determines how dark the playfield is outside of dungeons/battles
Dim DarknessSpeed
'DarknessSpeed				 	= 175 ' determines how quickly the table gets dark (lower number equals faster)
Dim EnableFlicker':EnableFlicker 			= True
Dim MagnetTime':MagnetTime				= 30			' Magnet stays on until right magna save key releases it, or timer runs out value is in seconds
' Magnet variables if you do not have magna save keys
Dim UseRflipper':UseRflipper 				= False		' Control magnet - Turn on if you don't have magna save buttons
Dim AutoMagnetTime':AutoMagnetTime 			= 20		' Controls how many seconds the magnet is turned on for when right inlane is hit
Dim AutoMagnetOn ':AutoMagnetOn 				= False		' Controls if right inline turns on magnet automatically
'************************************************CHOOSE FPS***********************************************************************
Dim FPS':FPS 						= 1			' Slow PC Recommended "0" Fast PC Recommended 1
Dim ResetHighScore':ResetHighScore			= 0		'0 = Keep Scores.  1 = Reset all high scores.  START TABLE ONCE to reset high scores and then set this back to 0
...
Sub Table1_OptionEvent(ByVal eventId)
	...
	'Difficulty
	Difficulty = Table1.Option("Game Difficulty", 1, 3, 1, 1, 0, Array("Radiant-Easy (Default)", "Darkest-Medium", "Stygian-Hard"))
	SetDifficulty Difficulty

	' VR Room
'	VRRoomChoice = Table1.Option("VR Room Choice", 1, 2, 1, 1, 0, Array("1 - Cathedral (Defualt)", "2 - Sphere Dungeon"))
'	SetVRRoom VRRoomChoice

	'FPS	
	' Slow PC Recommended "0" Fast PC Recommended 1
	FPS = Table1.Option("Frames Per Second Adj", 0, 1, 1, 0, 0, Array("0 - Fast CPU (Default)", "1 - Slow CPU "))

	' Default ball
 ' Determines which ball is loaded as the default ball
									' 0 - Silver Glow
									' 1 - Purple Glow
									' 2 - Green Glow
									' 3 - Blue Glow
				'DEFAULT			' 4 - Silver NO Glow
									' 5 - Blue Glow
									' 6 - Flaming Glow
									' 7 - Yellow Glow
									' 8 - Gold Glow
									' 9 - Pink Glow
'	DefaultBall = Table1.Option("Default Ball", 0, 9, 1, 4, 0, Array("0-9"," 4 is Default"))

	'Scorbit
'	ScorbitActiveOpt = Table1.Option("Scorbit Active", 0, 1, 1, 0, 0, Array("0 - Off", "1 - On"))
'	SetScorbitActive ScorbitActiveOpt

	'Staged Flipper
	'0 = Normal  1 = Staged Flippers
	StagedFlippers = Table1.Option("Staged Flippers", 0, 1, 1, 0, 0, Array("0 - Off", "1 - On"))
	'SetStagedFlippers

	'Max Dungeon Darkness
	MaxDarkness = Table1.Option("Max Dungeon Darkness", 80, 99, 1, 95, 0, Array("80-99"," 95 (Default)"))

	'Default Dungeon DarknessLayer
	' determines how dark the playfield can get (higher number equals darker
	DefaultDarkness = Table1.Option("Starting Dungeon Darkness", 0, 80, 1, 5, 0, Array("0-80"," 5 (Default)"))

	'Balls Per Game
	' Set How Many Balls Per Game
	BallsPerGame = Table1.Option("Balls Per Game", 3, 5, 1, 3, 0, Array("3 (Default)","4", "5"))
	SetBallsPerGame BallsPerGame


	'Enable GI flicker
	EnableFlicker = Table1.Option("Enable GI Flicker", 0, 1, 1, 1, 0, Array("0 - Off", "1 - On (Default)"))
'	EnableFlicker = 1

	'Magnet Time
	' Magnet stays on until right magna save key releases it, or timer runs out value is in seconds
	MagnetTime = Table1.Option("Magnet Duration", 0, 999, 1, 30, 0, Array("0-999"," 30 (Default)"))

	'Auto Magnet Time
	' Controls how many seconds the magnet is turned on for when right inlane is hit
	AutoMagnetTime = Table1.Option("Auto Magnet Duration", 0, 99, 1, 20, 0, Array("0-99"," 20 (Default)"))

	'AutoMagnet On
	' Controls if right inline turns on magnet automatically
	AutoMagnetOn = Table1.Option("Auto Magnet from Inlane", 0, 1, 1, 0, 0, Array("0 - Off (Default)", "1 - On "))

	'Wood Ramps
	WoodRamps = Table1.Option("Type of Ramps", 0, 1, 1, 1, 0, Array("0 - Bronze", "1 - Wood (Default) "))
	SetWoodRamps WoodRamps

	'Use R Flipper
	' Control magnet - Turn on if you don't have magna save buttons
	UseRFlipper = Table1.Option("Use Right Flipper For Magnet", 0, 1, 1, 0, 0, Array("0 - Off (Default)", "1 - On "))

	FlipperTricksOpt = Table1.Option("Flipper Tricks Difficulty", 0, 1, 1, 1, 0, Array("Easy (Keyboard & Gamepad)", "Standard (Optimal for leaf switches)"))
	SetFlipperTricks FlipperTricksOpt

	'LUT Selector
	' 1 - on; 0 - off; if on then the magnasave button let's you rotate all LUT's
	'EnableLUTSelector = Table1.Option("Enable LUT Selection", 0, 1, 1, 1, 0, Array("False ", "True (Default)"))
	'SetLUT EnableLUTSelector	

	' Reset High Scores
	'0 = Keep Scores.  1 = Reset all high scores.  START TABLE ONCE to reset high scores and then set this back to 0
	ResetHighScore = Table1.Option("Reset High Scores", 0, 1, 1, 0, 0, Array("0 - Keep Scores", "1 - Reset Scores (Must Save & Quit) "))
...
Dim BallsPerGame     ' actually redefined in setballspergame routine 
...
Sub SetDifficulty(Opt)
	...
	if Scorbit.bSessionActive then
		GameModeStrTmp="D{red}:Difficulty: "&Opt
...
Sub LightEffect(n)
	if Mode(CurrentPlayer,0) = 6 then Exit Sub
    Select Case n
        Case 0 ' all off
...
Sub CreateNewBall()
	if Scorbit.bSessionActive then
		GameModeStrTmp="B{red}:Ball: "&Balls
		Scorbit.SetGameMode(GameModeStrTmp)
	End If
...
'				triggerscript 300, "DisplaySelection"
				DisplaySelection
...
'ModeFail
	if Scorbit.bSessionActive then
		GameModeStrTmp="B{red}:Lost Ball "&Balls
		Scorbit.SetGameMode(GameModeStrTmp)
	End If
...
'				triggerscript 300, "DisplaySelection"
				DisplaySelection
				' new ball is created once a selection is made
			Elseif PartySize(CurrentPlayer) = 0 Then ReturnToVillage:CreateNewBall:Debug "WTFWTF"
			Else ' AM state 0 or 1 and all party members still alive
...
GetAM
	Select Case ActiveMode
		...
		Case 1: ModeTitle = "Assemble Party"
		Case 2: ModeTitle =  "Ruins Dungeon"
		Case 3: ModeTitle = "Assemble Party"
		Case 4: ModeTitle = "Cove Dungeon"
		Case 5: ModeTitle = "Assemble Party"
		Case 6: ModeTitle = "Weald Dungeon"
		Case 7: ModeTitle = "Assemble Party"
		Case 8: ModeTitle = "Warrens Dungeon"
		Case 9: ModeTitle = "Wizard Lock"
		Case 10: ModeTitle = "Prophet Battle"
		Case 11: ModeTitle = "Drowned Crew"
		Case 12: ModeTitle = "Vvulf Battle"
		Case 13: ModeTitle = "Swine Prince"
		Case 14: ModeTitle = "Shambler Battle"
...
Sub HideScorbit
	pbackglasslabelhide "ScorbitQRIcon1"  ' make sure Qr Claim is hidden
	pbackglasslabelhide "ScorbitQR1" ' make sure Qr Claim is hidden
	pbackglasslabelhide "ScorbitQRIcon2"  ' make sure Qr Claim is hidden
	pbackglasslabelhide "ScorbitQR2" ' make sure Qr Claim is hidden
End Sub
...
GetAM
	...
	Select Case ActiveMode
		...
		Case 14: CurrentDungeonBG = "PuPevent 982"

		Case 0,9,15: ' Do nothing
		...
		Select case tmp2
			...
			Case 0: ' Medicine
					...
					Else
						...
						CreateItemString
						if Scorbit.bSessionActive then
							GameModeStrTmp="Item{Purple}:Medicine Added"
							Scorbit.SetGameMode(GameModeStrTmp)
						End If
					End If
			Case 1: ' Bandages
					...
					Else
						...
						CreateItemString
						if Scorbit.bSessionActive then
							GameModeStrTmp="Item{Purple}:Bandages Added"
							Scorbit.SetGameMode(GameModeStrTmp)
						End If
					End If
			Case 2: ' Key
					...
					Else
						...
						CreateItemString
						if Scorbit.bSessionActive then
							GameModeStrTmp="Item{Purple}:Key Added"
							Scorbit.SetGameMode(GameModeStrTmp)
						End If
					End If
			Case 3: ' Shovel
					...
					Else
						...
						CreateItemString
						if Scorbit.bSessionActive then
							GameModeStrTmp="Item{Purple}:Shovel Added"
							Scorbit.SetGameMode(GameModeStrTmp)
						End If
					End If
			Case 4: ' Potion
					...
					Else
						...
						CreateItemString
						if Scorbit.bSessionActive then
							GameModeStrTmp="Item{Purple}:Potion Added"
							Scorbit.SetGameMode(GameModeStrTmp)
						End If
					End If
			Case Else:
	...
	ElseIf Mode(CurrentPlayer,0) = 4 Then 
		TotalMonsterHits = 24
			...
			if  RegionMonster(CurrentPlayer,4) > 23 Then
				...
				BallHandlingQueue.Add "KillPuPMonster-0","KillPuPMonster ""0""",70,3100,0,0,0,True
			...
			Elseif RegionMonster(CurrentPlayer,4) > 17 Then
				...
				BallHandlingQueue.Add "KillPuPMonster-1","KillPuPMonster ""1""",70,3100,0,0,0,True
			...
			Elseif RegionMonster(CurrentPlayer,4) > 11 Then
				...
				BallHandlingQueue.Add "KillPuPMonster-2","KillPuPMonster ""2""",70,3100,0,0,0,True
			...
			Elseif RegionMonster(CurrentPlayer,4) > 5 Then
				...
				BallHandlingQueue.Add "KillPuPMonster-3","KillPuPMonster ""3""",70,3100,0,0,0,True
...
End If
	...
	if Scorbit.bSessionActive then
		Select Case tmpChar
			Case 1
				GameModeStrTmp="PM{Green}:Added GraveRobber"
			Case 2
				GameModeStrTmp="PM{Green}:Added Highwayman"
			Case 3
				GameModeStrTmp="PM{Green}:Added Jester"
			Case 4
				GameModeStrTmp="PM{Green}:Added Vestal"
			Case 5
				GameModeStrTmp="PM{Green}:Added PlagueDoctor"
			Case 6
				GameModeStrTmp="PM{Green}:Added Occultist"
			Case 7
				GameModeStrTmp="PM{Green}:Added Crusader"
			Case 8
				GameModeStrTmp="PM{Green}:Added HoundMaster"
		End Select
		Scorbit.SetGameMode(GameModeStrTmp)
	End If


	Debug "Calling AddPartyMember:"&PartySize(CurrentPlayer) & ":" & NextChar
	if PartySize(CurrentPlayer) < 4 Then
		...
		Select Case tmpChar
			Case 1:
					...
					CheckParty
					GeneralPupQueue.Add "Pup817","pupevent 817",30,0,0,0,GenPuPExpiry,False
...
'							Debug " Party at Max Size"
			...
			Case 2:
					...
					PartySize(CurrentPlayer) = PartySize(CurrentPlayer) + 1
					CheckParty
					GeneralPupQueue.Add "Pup810","pupevent 810",30,0,0,0,GenPuPExpiry,False
...
'							Debug " Party at Max Size"
			...
			Case 3:
					...
					PartySize(CurrentPlayer) = PartySize(CurrentPlayer) + 1
					CheckParty
					GeneralPupQueue.Add "Pup811","pupevent 811",30,0,0,0,GenPuPExpiry,False
...
'							Debug " Party at Max Size"
			...
			Case 4:
					...
					PartySize(CurrentPlayer) = PartySize(CurrentPlayer) + 1
					CheckParty
					GeneralPupQueue.Add "Pup812","pupevent 812",30,0,0,0,GenPuPExpiry,False
...
'							Debug " Party at Max Size"
			...
			Case 5:
					...
					PartySize(CurrentPlayer) = PartySize(CurrentPlayer) + 1
					CheckParty
					GeneralPupQueue.Add "Pup813","pupevent 813",30,0,0,0,GenPuPExpiry,False
...
'							Debug " Party at Max Size"
			...
			Case 6:
					...
					PartySize(CurrentPlayer) = PartySize(CurrentPlayer) + 1
					CheckParty
					GeneralPupQueue.Add "Pup814","pupevent 814",30,0,0,0,GenPuPExpiry,False
...
'							Debug " Party at Max Size"
			...
			Case 7:
					...
					PartySize(CurrentPlayer) = PartySize(CurrentPlayer) + 1
					CheckParty
					GeneralPupQueue.Add "Pup815","pupevent 815",30,0,0,0,GenPuPExpiry,False
...
'					Debug " Added Hound "&PartySize(CurrentPlayer)
					GeneralPupQueue.Add "Pup816","pupevent 816",30,0,0,0,GenPuPExpiry,False
		...
		Select Case remChar
			...
			Case 0:
				if Party(CurrentPlayer,0) = 1 Then
					...
					if Scorbit.bSessionActive then
						GameModeStrTmp="PM{red}:GraveRobber Died"
						Scorbit.SetGameMode(GameModeStrTmp)
					End If
					HideCharIcons 0
...
'					Debug "Grave Not in Party"
			...
			Case 1:
				if Party(CurrentPlayer,1) = 1 Then
					...
					if Scorbit.bSessionActive then
						GameModeStrTmp="PM{red}:HighwayMan Died"
						Scorbit.SetGameMode(GameModeStrTmp)
					End If
					HideCharIcons 1
					If PartySize(CurrentPlayer) < 1 Then 
...
'					Debug "HwyMan Not in Party"
			...
			Case 2:
				if Party(CurrentPlayer,2) = 1 Then
					...
					if Scorbit.bSessionActive then
						GameModeStrTmp="PM{red}:Jester Died"
						Scorbit.SetGameMode(GameModeStrTmp)
					End If
					HideCharIcons 2
					If PartySize(CurrentPlayer) < 1 Then 
...
'					Debug "Jester Not in Party"
			...
			Case 3:
				if Party(CurrentPlayer,3) = 1 Then
					...
					if Scorbit.bSessionActive then
						GameModeStrTmp="PM{red}:Vestal Died"
						Scorbit.SetGameMode(GameModeStrTmp)
					End If
					HideCharIcons 3
					If PartySize(CurrentPlayer) < 1 Then 
...
'					Debug "Healer Not in Party"
			...
			Case 4:
				if Party(CurrentPlayer,4) = 1 Then
					...
					if Scorbit.bSessionActive then
						GameModeStrTmp="PM{red}:PlagueDoctor Died"
						Scorbit.SetGameMode(GameModeStrTmp)
					End If
					HideCharIcons 4
					If PartySize(CurrentPlayer) < 1 Then 
...
'					Debug "Plague DR Not in Party"
			...
			Case 5:
				if Party(CurrentPlayer,5) = 1 Then
					...
					if Scorbit.bSessionActive then
						GameModeStrTmp="PM{red}:Occultist Died"
						Scorbit.SetGameMode(GameModeStrTmp)
					End If
					HideCharIcons 5
					If PartySize(CurrentPlayer) < 1 Then 
...
'					Debug "Occultist Not in Party"
			...
			Case 6:
				if Party(CurrentPlayer,6) = 1 Then
					...
					if Scorbit.bSessionActive then
						GameModeStrTmp="PM{red}:Crusader Died"
						Scorbit.SetGameMode(GameModeStrTmp)
					End If
					HideCharIcons 6
					If PartySize(CurrentPlayer) < 1 Then 
...
'					Debug "Knight Not in Party"
			...
			Case 7:
				if Party(CurrentPlayer,7) = 1 Then
					...
					if Scorbit.bSessionActive then
						GameModeStrTmp="PM{red}:HoundMaster Died"
						Scorbit.SetGameMode(GameModeStrTmp)
					End If
					HideCharIcons 7
					If PartySize(CurrentPlayer) < 1 Then 
...
Debug "PartySting on Exit: " &PartyString
    ...
    BallsOnPlayfield = BallsOnPlayfield - 1
	Debug "Drain Hit- BoP: " & BallsOnPlayfield
...
'		Playsound	"vo_ballsave"
        ...
        Else  ' Not in ball saver mode
            ...
            If(BallsOnPlayfield = 0)Then		
				...
				if AMState < 3 And ActiveMode < 14 And ActiveMode > 0 Then	Debug "Calling ModeFail": ModeFail "drainhit"   ' Do not call modefail if completed   1.2b
				Light010.state = 0
				Debug "BOP 0"
				OldGI = "white"
                ChangeGi white
				...
				Debug " Calling Lost Ball EOB " & AMCombo
			End If
        End If
    End If
End Sub
...
Sub AddBonusMultiplier(n)
    ...
    if(BonusMultiplier(CurrentPlayer) + n <= MaxBonusMultiplier)then
		...
		if Scorbit.bSessionActive then
			GameModeStrTmp="BM{green}:Bonus Multiplier: " &NewBonusLevel
			Scorbit.SetGameMode(GameModeStrTmp)
		End If
...
Sub AddPlayfieldMultiplier(n)
 ...
 '       DMD "_", CL("PLAYFIELD X " &NewPFLevel), "_", eNone, eBlink, eNone, 2000, True, "sfx_thunder" &RndNbr(7)
		...
		if Scorbit.bSessionActive then
			GameModeStrTmp="PM{Blue}:Increased PF Multiplier: " &NewPFLevel
			Scorbit.SetGameMode(GameModeStrTmp)
		End If
    ' Play a voice sound
...
Sub DecreasePlayfieldMultiplier 'reduces by 1 the playfield multiplier
    ...
    if(PlayfieldMultiplier(CurrentPlayer) > 1)then
		...
		if Scorbit.bSessionActive then
			GameModeStrTmp="PM{red}:Decreased PF Multiplier: " &NewPFLevel
			Scorbit.SetGameMode(GameModeStrTmp)
		End If
...
Sub AwardExtraBall()
 ...
 '   DMD "_", CL("EXTRA BALL WON"), "_", eNone, eBlink, eNone, 1000, True, SoundFXDOF("Knocker", 108, DOFPulse, DOFKnocker)
	...
	if Scorbit.bSessionActive then
		GameModeStrTmp="EB{purple}:Extraball Awarded"
		Scorbit.SetGameMode(GameModeStrTmp)
	End If
...
Sub AwardSpecial()
 '   DMD "_", CL("EXTRA GAME WON"), "_", eNone, eBlink, eNone, 2000, True, SoundFXDOF("Knocker", 108, DOFPulse, DOFKnocker)
	...
	if Scorbit.bSessionActive then
		GameModeStrTmp="D{purple}:Special Awarded"
		Scorbit.SetGameMode(GameModeStrTmp)
	End If
	'ScorbitBuildGameModes
End Sub
...
Sub AwardSkillshot()
 ...
 '   DMD CL("SKILLSHOT"), CL(FormatScore(SkillshotValue(CurrentPlayer))), "d_border", eNone, eBlinkFast, eNone, 2000, True, "vo_skillshot"
	...
	if Scorbit.bSessionActive then
		GameModeStrTmp="SS{blue}:Skillshot"
		Scorbit.SetGameMode(GameModeStrTmp)
	End If
End Sub
...
Sub AwardSuperSkillshot()
 ...
 '   DMD CL("SUPER SKILLSHOT"), CL(FormatScore(SuperSkillshotValue(CurrentPlayer))), "d_border", eNone, eBlinkFast, eNone, 2000, True, "vo_superskillshot"
    ...
    LightEffect 2
	if Scorbit.bSessionActive then
		GameModeStrTmp="SS{Yellow}:Super Skillshot"
		Scorbit.SetGameMode(GameModeStrTmp)
	End If
End Sub
...
Sub DisplayOverlayCounts
	...
	Elseif bSupressDMD = True Then
		BallHandlingQueue.Add "DisplayOverlayCounts","DisplayOverlayCounts",70,1000,0,0,0,False
	Else
		'Debug "in display overlay counts"
...
Debug2 "in EOB RETURN- bDrain: "& bDrain
...
if BSMode = 1 or GMode = 1 or TMode = 1 Then Exit Sub
	...
	Select case ActiveMode  ' Change preview char each time cball is hit
		Case 0:
			...
			Debug "CBALL HIT MOD: " &CBHits(CurrentPlayer) mod 10
				if DMDType <> 2 Then 
					pup2helper 3000
				Else
					HideDMD 3005
...
'		PuPEvent 966   ' preventambush video?
		If Party(CurrentPlayer,5) = 1 Then
			...
			if Scorbit.bSessionActive then
				GameModeStrTmp="Item{Orange}:Occulist Ability Used - Ambush Destroyed"
				Scorbit.SetGameMode(GameModeStrTmp)
			End If
...
Debug "ApplyGrave: " & InBattle &":" & BID
	if InBattle = 1 AND BattleID(CurrentPlayer) = OrigGraBID then
		if Party(CurrentPlayer,0) = 1 And PartySize(CurrentPlayer) < 4 Then ' 
			...
			if Scorbit.bSessionActive then
				GameModeStrTmp="Item{Orange}:GraveRobber Ability Used"
				Scorbit.SetGameMode(GameModeStrTmp)
			End If
...
Debug "ApplyHwy: " & InBattle &":" & OrigHwyBID
	if InBattle = 1 AND BattleID(CurrentPlayer) = OrigHwyBID then
		if Party(CurrentPlayer,1) = 1 And bMultiBallMode = 0 And bBallinPlungerLane = False Then ' Hwy - Make sure multiball mode is false
			...
			if Scorbit.bSessionActive then
				GameModeStrTmp="Item{Orange}:Highwayman Ability Used"
				Scorbit.SetGameMode(GameModeStrTmp)
			End If
		...
		Debug "ApplyJester: PASSED"
			...
			Select Case jst
				Case 1,2,3,4,5,6,7:
					...
					AddItem
					if Scorbit.bSessionActive then
						GameModeStrTmp="Item{Orange}:Jester Ability Used - Added 2 Items"
						Scorbit.SetGameMode(GameModeStrTmp)
					End If
				Case Else:
					if Scorbit.bSessionActive then
						GameModeStrTmp="Item{Orange}:Jester Ability Used - Removed Trust Post"
						Scorbit.SetGameMode(GameModeStrTmp)
					End If
...
Debug "ApplyHealer: " & InBattle &":" & BID
	If InBattle = 1 AND BattleID(CurrentPlayer) = BID Then
		...
		Debug "ApplyHealer: PASSED"
			...
			if Scorbit.bSessionActive then
				GameModeStrTmp="Item{Orange}:Vestal Ability Used"
				Scorbit.SetGameMode(GameModeStrTmp)
			End If
...
Debug "ApplyPlague: " & InBattle &":" & BID
	If InBattle = 1  AND BattleID(CurrentPlayer) = BID Then
		...
		Debug "ApplyPlague: PASSED"
			...
			ResetTorches
			if Scorbit.bSessionActive then
				GameModeStrTmp="Item{Orange}:Plague Doctor Ability Used"
				Scorbit.SetGameMode(GameModeStrTmp)
			End If
		Else
			PlagueTimer.Enabled = 0
...
'				AudioQueue.Add "Pup256","PuPEvent 256",32,0,0,0,AudioExpiry,False
			...
			Else
				...
				if Scorbit.bSessionActive then
					GameModeStrTmp="Item{Orange}:Crusader Ability Used"
					Scorbit.SetGameMode(GameModeStrTmp)
				End If
			...
			Select Case ActiveMode
				Case 0:
					...
					if PartySize(CurrentPlayer) < 4 Then 
						...
						GeneralPupQueue.Add "Pup400","pupevent 400",94,0,0,0,0,True
						GeneralPuPQueue.Add "Pup819","PuPEvent 819",25,0,0,0,0,True					
					Else ' party is full
						BallHandlingQueue.Add "InnKickOut","InnKickBallOut",70,KickOutDelay,0,0,0,True
						li038.state = 0
						li047.state = 0
...
If TotalItemCount > 0 And AMState = 1 Then
 ...
 '               DMD CL("MYSTERY "), CL("EXTRA BALL IS LIT"), "_", eNone, eBlink, eNone, 2500, True, "vo_extraballislit"
				if NOT bExtraBallWonThisBall Then
					...
					if Scorbit.bSessionActive then
						GameModeStrTmp="Item{Blue}:Medicine Used"
						Scorbit.SetGameMode(GameModeStrTmp)
					End If
				Else				
...
'				DMD CL("MYSTERY "), CL(FormatScore(250000)), "_", eNone, eBlink, eNone, 2000, True, ""
				...
				BallHandlingQueue.Add "KickOut","KickBallOut",70,4600,0,0,0,True
					if Scorbit.bSessionActive then
						GameModeStrTmp="Item{Blue}:Bandages Used"
						Scorbit.SetGameMode(GameModeStrTmp)
					End If
			Else
				randomchest
...
'				DMD CL("MYSTERY "), CL(FormatScore(tmp)), "_", eNone, eBlink, eNone, 2000, True, ""
				...
				BallHandlingQueue.Add "KickOut","KickBallOut",70,4600,0,0,0,True
					if Scorbit.bSessionActive then
						GameModeStrTmp="Item{Blue}:Key Used"
						Scorbit.SetGameMode(GameModeStrTmp)
					End If
			Else
				randomchest			
...
'				DMD "_", CL("PLAYFIELD X 2"), "_", eNone, eBlinkFast, eNone, 2000, True, ""
				...
				BallHandlingQueue.Add "KickOut","KickBallOut",70,4600,0,0,0,True
					if Scorbit.bSessionActive then
						GameModeStrTmp="Item{Blue}:Shovel Used"
						Scorbit.SetGameMode(GameModeStrTmp)
					End If
			Else
				randomchest
...
'				DMD CL("MYSTERY "), CL(FormatScore(250000)), "_", eNone, eBlink, eNone, 2000, True, ""
				...
				BallHandlingQueue.Add "KickOut","KickBallOut",70,4600,0,0,0,True
					if Scorbit.bSessionActive then
						GameModeStrTmp="Item{Blue}:Vial Used"
						Scorbit.SetGameMode(GameModeStrTmp)
					End If
			Else
				if TotalItemCount > 1 Then
	...
	'			DMD CL("MYSTERY "), CL("1"), "_", eNone, eBlink, eNone, 2500, True, "vo_specialislit"
				...
				BallHandlingQueue.Add "KickOut","KickBallOut",70,4600,0,0,0,True
					if Scorbit.bSessionActive then
						GameModeStrTmp="Item{Red}:Heart Attack"
						Scorbit.SetGameMode(GameModeStrTmp)
					End If
			Else
...
'				pNote "Crazy", "Flippers"
				...
				if Party(CurrentPlayer,4) <> 1 Then
					...
					BallHandlingQueue.Add "KickOut","KickBallOut",70,4600,0,0,0,True
					if Scorbit.bSessionActive then
						GameModeStrTmp="Item{Red}:Madness"
						Scorbit.SetGameMode(GameModeStrTmp)
					End If
				Else
...
Debug "Checking for Ball Lock or Multiball"
	...
	'				DMD "_", CL("BACK UP ARRIVES"), "_", eNone, eNone, eNone, 1500, True, "vo_backup" 
					 ...
					 li039.state=0
									'ScorbitBuildGameModes
					if Scorbit.bSessionActive then
						GameModeStrTmp="MB{Blue}:Dungeon Multiball"
						Scorbit.SetGameMode(GameModeStrTmp)
					End If
				Else
...
Debug "---------- In Mode End"
	...
	Debug "ModeEnd ActiveMode: " &ActiveMode
	LastMode = ActiveMode
	Light010.State = 2
...
Sub WizardModePrep
    ...
    DOF 247, DOFOn
	if bRetry = False Then BallsInLock(CurrentPlayer) = 0 ' reset locked balls on first pass
...
Sub WizardMode
	...
	GeneralPupQueue.Add "Pup200","pupevent 200",90,0,0,0,0,True
	if DMDType <> 2 Then pupflasher1.visible=true
	GeneralPuPQueue.Add "Pup313","PuPEvent 313",14,100,0,0,0,True
	...
	triggerscript 6500, "addmultiball 3"
End Sub
...
Debug5 "In Check Orbits R/L/I - " &ROHIT &":" &LOHIT &":" &INHIT
...
'' ROUTINES FOR FULL ORBIT HITS
	...
	if LOHIT And ROHIT Then
		...
		debug5 " Full Orbit Hit L&R"
		ROHIT = 0
		LOHIT = 0
	End If

	if LOHIT And INHIT Then
		Select Case ActiveMode
			Case 1,3,5,7:
				AddPartyMember
			Case 2:
	...
	Select Case ActiveMode
		...
		Case 15:
			if bSuperJackpotReady = True Then
				...
				bSuperJackpotReady = False
				if Scorbit.bSessionActive then
					GameModeStrTmp="SJ{red}:Super Jackpot"
					Scorbit.SetGameMode(GameModeStrTmp)
				End If
			End If
			BallHandlingQueue.Add "KickOut","KickBallOut",70,KickOutDelay,0,0,0,True
...
Sub WinInTheDark
	...
	if Scorbit.bSessionActive then
		GameModeStrTmp="SM{yellow}:InTheDark Mode Completed"
		Scorbit.SetGameMode(GameModeStrTmp)
	End If
	...
	if Missions(CurrentPlayer,1) = 7 Then   ' Blacksmith Mode Finish Check
		...
		bSupressBGMessages = False				
			if bBSAll = 1 Then 
				...
				if Scorbit.bSessionActive then
					GameModeStrTmp="SM{red}:Blacksmith Mode Completed"
					Scorbit.SetGameMode(GameModeStrTmp)
				End If
			End If
...
Sub GambleCollect
	...
	if Scorbit.bSessionActive then
		GameModeStrTmp="SM{green}:Gambler Mode Completed"
		Scorbit.SetGameMode(GameModeStrTmp)
	End If
...
'		if Scorbit.DoInit(4112, "PupOverlays", myVersion, "DDUN") then 	' Staging
		if Scorbit.DoInit(4351, "PupOverlays", myVersion, "ddun-vpin") then 	' Prod
...
Sub PuPEvent(EventNum)
	...
	if bEOBSupress = True Then 
		Select Case True
			Case EventNum = 445 or EventNum = 511 or EventNum = 555 or EventNum = 890:
			...
			Case (EventNum > 299 And EventNum < 311): ' DMD guides
			Case Else:
				Debug2 "***************************"
	...
	if GMode = 1 Then
		Select Case EventNum
			...
			Case 831,833,837,838,300,400,298,433:
	...
	Elseif BSMode = 1 Then
		Select Case EventNum
			Case 824:
				PuPlayer.B2SData "D286",1  'send AUDIO version of event to puppack driver
			Case 292,830,835,836,300,400,297,432:
				PuPlayer.B2SData "D"&EventNum,1  'send event to puppack driver  
		End Select
		Exit Sub
	Elseif TMode = 1 Then
		Select Case EventNum
			Case 824:
				PuPlayer.B2SData "D286",1  'send AUDIO version of event to puppack driver
			Case 834,839,840,300,400,299,434:
				PuPlayer.B2SData "D"&EventNum,1  'send event to puppack driver  
		End Select
		Exit Sub  
...
'				triggerscript 5200, "InEvent = 0 "
			...
			Case 910,911,912,913,914,915,916,918:
				Debug "InEvent: " &EventNum
'				InEvent = 1
'				triggerscript 4400, "InEvent = 0 "
			...
			Case 941,942,943,944,945,946,947,948:
				Debug "InEvent: " &EventNum
'				InEvent = 1
...
'	PuPlayer.LabelNew pBackglass,"Mode",               dmdscr,7,33023   ,0,1,1,86.5,6.4,1,0
...
Sub UpdateAll
	debug2 "Called UpdateAll"
	if bEOBSupress = False And DMDType <> 0 Then
...
Sub UpdateDMD
'exit sub
Debug2 "In UpdateDMD - Supress? " &bSupressDMD &":" &bDecision
...
Sub ScorbitBuildGameModes()		' Custom function to build the game modes for better stats 
	dim GameModeStr
	if Scorbit.bSessionActive=False then Exit Sub 
	GameModeStr="Mode{blue}:In The Village"

	if BallsRemaining(CurrentPlayer) <= 0 Then	'no balls left
		GameModeStr="Mode{red}:YOU FAILED!!!"
	Else										'game on
		GetAM
		Debug4 "AMCombo Value - " &AMCombo
		Select Case AMCombo
			Case "1:1","3:1","5:1","7:1"
				Debug4 "Assemble Party"
				GameModeStr="Mode{yellow}:Assemble Your Party"
			Case "1:2","3:2","5:2","7:2"
				Debug4 "Party Assembled"
				GameModeStr="Mode{Purple}:Party Assembled"
			Case "2:1":
				Debug4 "Entered Ruins Dungeon"
				GameModeStr="Mode{yellow}:Entered Ruins Dungeon"			
			Case "2:2": 
				Debug4 "Ruins Dungeon Cleared"
				GameModeStr="Mode{yellow}:Ruins Dungeon Cleared"
			Case "10:2":
				Debug4 "Prophet Defeated"
				GameModeStr="Boss{purple}:Prophet Defeated"
			Case "4:1": 
				Debug4 "Entered Cove Dungeon"
				GameModeStr="Mode{blue}:Entered Cove Dungeon"
			Case "4:2": 
				Debug4 "Cove Dungeon Cleared"
				GameModeStr="Mode{blue}:Cove Dungeon Cleared"
			Case "11:2":
				Debug4 "DrownedCrew Defeated"
				GameModeStr="Boss{blue}:Drowned Crew Defeated"
			Case "6:1": 
				Debug4 "Entered Weald Dungeon"
				GameModeStr="Mode{red}:Entered Weald Dungeon"
			Case "6:2": 
				Debug4 "Weald Dungeon Cleared"
				GameModeStr="Mode{red}:Weald Dungeon Cleared"
			Case "12:2":
				Debug4 "Vvulf Defeated"
				GameModeStr="Boss{red}:Vvulf Defeated"
			Case "8:1": 
				Debug4 "Entered Warrens Dungeon"
				GameModeStr="Mode{orange}:Entered Warrens Dungeon"
			Case "8:2": 
				Debug4 "Warrens Dungeon Cleared"
				GameModeStr="Mode{orange}:Warrens Dungeon Cleared"
			Case "13:2":
				Debug4 "SwinePrince Defeated"
				GameModeStr="Boss{pink}:Swine Prince Defeated"
			Case "14:2":
				Debug4 "Shambler Defeated"
				GameModeStr="Boss{purple}:Shambler Defeated"
			Case "15:1": 
				Debug4 "WizardMode Achieved"
				GameModeStr="Wiz{green}:Wizard Mode"
			Case Else:
				Debug4 "Nothing Mapped"
				GameModeStr="Mode{blue}:In The Village"
				'Do nothing
		End Select
...
Sub SetStagedFlippers(Opt)
	Select Case Opt
		Case 0: stagedFlippers = 0
		Case 1:	stagedFlippers = 1
	End Select
End Sub

Sub SetScorbitActive(Opt)

Debug2 "Setting Scorbit" &Opt
	Select Case Opt
		Case 0: ScorbitActive = 0
		Case 1:	ScorbitActive = 1: CheckPairing
	End Select
End Sub
...
Sub SetWoodRamps(Opt)
	Select Case Opt
		Case 0: WoodRamps = False
		Case 1:	WoodRamps = True
	End Select
...
Sub SetGIFlicker(Opt)
	Select Case Opt
		Case 0: EnableFlicker = False
		Case 1:	EnableFlicker = True
	End Select
End Sub

Sub SetUserRFlipper(Opt)
	Select Case Opt
		Case 0: UseRFlipper = False
		Case 1:	UseRFlipper = True
	End Select
End Sub

Sub SetAutoMagnetOn(Opt)
	Select Case Opt
		Case 0: AutoMagnetOn = False
		Case 1:	AutoMagnetOn = True
	End Select
End Sub

Sub SetEnableLUTSelector(Opt)
	Select Case Opt
		Case 0: EnableLUTSelector = False
		Case 1:	EnableLUTSelector = True
	End Select
End Sub
