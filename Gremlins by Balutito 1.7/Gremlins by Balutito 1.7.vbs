...
Const UseFlexDMD = 0	' 1 is on - for real dmd's mainly but will work on lcd. 0 is off.
...
Sub Victory_Init
    ...
    With Controller
		...
		.SplashInfoLine="Gremlins By Balutito"
		.HandleKeyboard=0
		.ShowTitle=0
		.ShowDMDOnly=1
		.ShowFrame=0
		.HandleMechanics=0
		.Hidden=Hidden
		.Games("victory").Settings.Value("sound") = 0
		.Games("victory").Settings.Value("samples") = 0
		.SolMask(0) = 0
	On Error Resume Next
		.Run GetPlayerHWnd
	End With
	...
	With bsHoleKicker
		.InitSaucer HoleKicker, swHoleKicker, 155, 12
	End With
...
Sub FlexDMD_Init() 'default/startup values
	...
	If Not FlexDMD is Nothing Then
		...
		FlexDMD.TableFile = victory.Filename & ".vpx"
		...
		With FlexDMDScene
			...
			.AddActor FlexDMD.NewImage("HiScores", "VPX.DMD_HiScoreLogo")
			.GetImage("HiScores").Visible = False
...
music = "gremlins.mp3"
PlayMusic "gremlins.mp3" 

Sub Victory_MusicDone()
PlayMusic "gremlins.mp3"
End Sub 
...
Sub TopKicker_Hit(): PlaySoundAtVol "KickerEnter", TopKicker, 0.15: bsTopKicker.AddBall Me: End Sub
Sub RightKicker_Hit(): PlaySoundAtVol "KickerEnter", RightKicker, 0.15: bsRightKicker.AddBall Me: End Sub
Sub HoleKicker_Hit(): PlaySoundAtVol "KickerEnter", HoleKicker, 0.15: bsHoleKicker.AddBall Me: End Sub
...
'****						      Targets                 						****
'*********************************************************************************** 
Sub sw70_Hit:vpmTimer.PulseSw(swLeftSpotTarget):PlaySound SoundFX("Target",DOFTargets):End Sub
Sub sw71_Hit:vpmTimer.PulseSw(swRightSpotTarget):PlaySound SoundFX("Target",DOFTargets):End Sub
Sub sw40_Hit:vpmTimer.PulseSw(swSpotTarget1):PlaySound SoundFX("Target",DOFTargets):End Sub
Sub sw41_Hit:vpmTimer.PulseSw(swSpotTarget4):PlaySound SoundFX("Target",DOFTargets):End Sub
Sub sw50_Hit:vpmTimer.PulseSw(swSpotTarget2):PlaySound SoundFX("Target",DOFTargets):End Sub
Sub sw51_Hit:vpmTimer.PulseSw(swSpotTarget5):PlaySound SoundFX("Target",DOFTargets):End Sub
Sub sw60_Hit:vpmTimer.PulseSw(swSpotTarget3):PlaySound SoundFX("Target",DOFTargets):End Sub
Sub sw61_Hit:vpmTimer.PulseSw(swSpotTarget6):PlaySound SoundFX("Target",DOFTargets):End Sub
...
Sub sw53_Spin():PlaySoundAtVol "fx_Spinner", sw53, 0.15:vpmTimer.PulseSw (swLeftSpinner):End Sub
Sub sw54_Spin():PlaySoundAtVol "fx_Spinner", sw54, 0.15:vpmTimer.PulseSw (swRightSpinner):End Sub
...
Sub Backdrop_Init
	...
	Else
		...
		if UseFlexDMD then LedsTimer.Enabled = 1
	End If
End Sub
...
Sub UpdateFlexChar(id, value)
	...
	If id < 40 Then
		...
		if id = 8 Then
			if value = 17456 Then
				FlexDMDScene.GetImage("HiScores").Visible = True
			Else
				FlexDMDScene.GetImage("HiScores").Visible = False
			end if
		end if
...
Sub Victory_Exit()
    ...
    Controller.Stop
    Controller.Games("victory").Settings.Value("sound") = 1
    Controller.Games("victory").Settings.Value("samples") = 1
	If UseFlexDMD then
		If Not FlexDMD is Nothing Then 
			FlexDMD.Show = False
			FlexDMD.Run = False
