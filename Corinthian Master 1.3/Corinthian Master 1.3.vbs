...
Sub BallReleaseTimer_Timer
	If GameStarted = True Then
		If BIP < maxBalls Then
			...
			BallRelease.CreateSizedBall(BallSize).UserValue = BIP
...
Sub Addpoints(ScorePar)
		if GameStarted Then			
			...
			If ActiveBall.UserValue = BIP Then CurrentBall = True Else CurrentBall = False End If 
		end If
		If ScorePar > 1 Then
			PlayerScore = PlayerScore + ScorePar
			debug.print "Ball " & ActiveBall.UserValue & " hit       " & ScorePar & " =" & PlayerScore
			If CurrentBall = True Then NextBall:End If
		ElseIf ScorePar = 0 Then
			debug.print "Ball " & ActiveBall.UserValue & " hit end   " & ScorePar & " =" & PlayerScore & " - " & TriggerEndofPlayfield.BallCntOver & " lost"
			If CurrentBall = True Then NextBall:End If
		Elseif ScorePar = -1 Then
			debug.print "Ball " & ActiveBall.UserValue & " unhit end " & ScorePar & " =" & PlayerScore
		Else
			PlayerScore = PlayerScore + ScorePar
			debug.print "Ball " & ActiveBall.UserValue & " hit       " & ScorePar & " =" & PlayerScore
		End If
