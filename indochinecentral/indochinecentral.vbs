...
cPuPPack = "indochine central tour"    ' name of the PuP-Pack / PuPVideos folder for this table
...
Const cGameName = "indochine central tour"
...
Sub SphereTimer_Timer
   sphere.roty = sphere.roty + 1
   disco.rotz = disco.rotz + 1
   if sphere.roty > 360 then
       sphere.roty = 1
   end if
end sub

Sub foule_hit()
PlaySound "fouleapplause"
End Sub
...
Sub holotimer_timer
    ...
    indocourt2.imageA = "indocourt2-" &i2
...
Sub EndOfBall()
    ...
    If NOT Tilted Then
        ...
        DMD CL(0, "SWITCHES I " & SwitchBonusCount), CL(1, FormatScore(AwardPoints)), "", eNone, eNone, eNone, 1000, True, ""

        'Ilussia letters
        AwardPoints = IlussiaBonusCount * 25000
        TotalBonus = TotalBonus + AwardPoints
        DMD CL(0, "INDOCHINE LETTERS X " & IlussiaBonusCount), CL(1, FormatScore(AwardPoints)), "", eNone, eNone, eNone, 1000, True, ""

        'Deathblows
        AwardPoints = DeathblowBonusCount * 45000
        TotalBonus = TotalBonus + AwardPoints
        DMD CL(0, "FANTASTIC CONCERT X " & DeathblowBonusCount), CL(1, FormatScore(AwardPoints)), "", eNone, eNone, eNone, 1000, True, ""
        ...
        TotalBonus = TotalBonus + AwardPoints
        DMD CL(0, "FULL SHOW X " & PyramidBonusCount), CL(1, FormatScore(AwardPoints)), "", eNone, eNone, eNone, 1000, True, ""

        'Mancha Xanandra completed
        AwardPoints = ManchaXBonusCount * 250000
        TotalBonus = TotalBonus + AwardPoints
        DMD CL(0, "CENTRAL TOUR " & ManchaXBonusCount), CL(1, FormatScore(AwardPoints)), "", eNone, eNone, eNone, 1000, True, ""
    ...
    if(dqHead = dqTail)Then
        ...
        ELSE
            Select Case Mode(CurrentPlayer, 0)
                Case 1:tmp1 = "INDOCHINE MULTIBALL"
                Case 2:tmp1 = "   NIKO MULTIBALL"
                Case 3:tmp1 = "CONCERT 13 MULTIBALL"
                Case 4:tmp1 = " BLACK CITY TOUR"
                Case 5:tmp1 = "INDOCHINE PARADIZE"
                Case 6:tmp1 = " INDOCHINE 13"
                Case 7:tmp1 = "  L AVENTURIER"
                Case 8:tmp1 = " ALICE ET JUNE"
                Case 9:tmp1 = "  CENTRAL TOUR"
                Case 10:tmp1 = "COLLECT GROUP MEMBERS"
                Case 11:tmp1 = " GOOD CONCERT"
                Case 12:tmp1 = "417000 SPECTACTORS"
            End Select
            tmp2 = ""
        End If
    End If
...
Sub InstantInfo
    ...
    DMD CL(0, "BACKSTAGE JACKPOT"), CL(1, FinisterraSJPValue(Currentplayer)), "", eNone, eNone, eNone, 800, False, ""
    tmp = FormatScore(SuperPyramidValue) & " X " & SuperPyramidMultiplier
    DMD CL(0, "SUPER SHOW VALUE"), CL(1, tmp), "", eNone, eNone, eNone, 800, False, ""
...
Sub Trigger008_Hit 'Left ramp done
    ...
    Select Case Mode(CurrentPlayer, 0)
        ...
        Case 4
            If Light054.State Then
                DMD "BLACK CITY COMBO", CL(1, FormatScore(CelticValue) & " X " & CelticMultiplier), "", eNone, eBlink, eNone, 1500, True, ""
                light054.State = 0
                light053.State = 2
            ...
            Else
                DMD "BLACK CITY RAMPSHOT", CL(1, FormatScore(CelticValue)), "", eNone, eBlink, eNone, 1500, True, ""
    ...
    Select Case Mode(CurrentPlayer, 0)
        ...
        Case 4
            If Light053.State Then
                ...
                CelticCombo.Enabled = 1
            Else
                DMD "BLACK CITY RAMPSHOT", CL(1, FormatScore(CelticValue)), "", eNone, eBlink, eNone, 1500, True, "vo_nicerampshot"
                AddScore CelticValue
                CelticValue = CelticValue + 150000
            End If
        Case 5
...
Sub AddBrujaLetter(n)
    ...
    if(BrujaLetter(CurrentPlayer) + n <= 5)then
        ...
        DMD "_", "A GROUP MEMBER FOUND", "", eNone, eBlink, eNone, 1500, True, "Fanfare4"
...
Sub UpdateBrujaLetter
    ...
    Select Case BrujaLetter(CurrentPlayer)
        Case 0:light064.State = 0:light065.State = 0:light066.State = 0:light067.State = 0:light068.State = 0:disco.visible = 0:indocourt2.visible = 0
        ...
        Case 5:light064.State = 2:light065.State = 2:light066.State = 2:light067.State = 2:light068.State = 2:disco.visible = 1:indocourt2.visible = 1
...
Sub WinAtlantia
    ...
    DMD "  YOU WON ATLANTIA", CL(1, FormatScore(tmp)), "", eNone, eBlink, eNone, 1500, True, "fanfare3"
    Addscore tmp
    StopAtlantia
    ResetBRUJALetter
    StartBRUJALEtter
...
Sub ArbolesTimer1_Timer 'countdown JP value
    ...
    DMD "  SHOOT TIME MACHINE", CL(1, FormatScore(ArbolesJPValue)), "", eNone, eNone, eNone, 200, True, ""
...
Sub CheckFolkCount
    ...
    Select Case FolkCount
        ...
        Case 4:DMD "_", "OPENING " &FolkCount& " TIME", "", eNone, eBlink, eNone, 1500, True, ""
            ...
            bAddaBallActivated = True
            Light046.State = 2
        Case 5:DMD "_", "SHOOT TIME TARGET", "", eNone, eBlink, eNone, 1500, True, ""
            Light054.State = 0
            Light053.State = 0
            Light057.State = 2
        Case 6:
            ...
            bAddaBallActivated = True
            Light046.State = 2
        Case 7:DMD "_", "SHOOT TIME TARGET", "", eNone, eBlink, eNone, 1500, True, ""
            Light054.State = 0
            Light053.State = 0
            Light057.State = 2
        Case 8:
            If LastSwitchHit = "bruja_target" Then
                ...
                Light057.State = 0
                DMD "_", "LOCK ON TIME MACHINE", "", eNone, eBlink, eNone, 1500, True, ""
                FolkTimer.Enabled = 1
            Else
                FolkCount = FolkCount -1
                Light054.State = 2
                ...
                Light057.State = 0
            End If
        Case 9:DMD "_", CL(1, "SHOOT TIME TARGET"), "", eNone, eBlink, eNone, 1500, True, ""
            FolkTimer.Enabled = 0
            For X = 0 to 7
                aLightArrows(x).State = 0
            Next
            Light057.State = 2
        Case 10:DMD "_", "ACTIVATE THE CONCERT", "", eNone, eBlink, eNone, 1500, False, ""
            DMD "_", "YOU WON THE CENTRAL TOUR", "", eNone, eBlink, eNone, 1500, True, ""
            StopFoltergeist
            ResetBRUJALetter
            StartBRUJALetter
    End Select
...
Sub StartIlussiaMUltiball
    StopBRUJALetter
    DMD CL(0, "INDOCHINE"), CL(1, "MULTIBALL"), "", eBlink, eBlink, eNone, 1500, True, "vo_multiball2" : pupevent 826
    AddMultiball 1 : pupevent 823
    post001.IsDropped = 0:PlaySoundAt "fx_SolenoidOn", Trigger010
    post002.IsDropped = 0
    vpmtimer.addtimer 2500, "PlaySoundAt""fx_SolenoidOff"", Trigger010: post001.IsDropped = 1:post002.IsDropped = 1 '"
...
Sub CheckLockedBalls 'updates also the lights
    Select Case BallsLocked(CurrentPlayer)
        ...
        Case 1:light007.State = 1:light006.State = 0:light070.State = 0
            LightEffect 1
            DMD CL(0, "CENTRAL TOUR"), CL(1, "BALL 1 LOCKED"), "", eNone, eBlink, eNone, 1500, True, "vo_ball1locked" : pupevent 814
            vpmtimer.addtimer 2500, "Exit_WitchNest '"
        Case 2:light007.State = 1:light006.State = 1:light070.State = 0
            LightEffect 1
            DMD CL(0, "CENTRAL TOUR"), CL(1, "BALL 2 LOCKED"), "", eNone, eBlink, eNone, 1500, True, "vo_ball2locked" : pupevent 815
            vpmtimer.addtimer 2500, "Exit_WitchNest '"
        Case 3:
            If Mode(CurrentPlayer, 0) = 10 Then
                DMD CL(0, "CENTRAL TOUR"), CL(1, "BALL 3 LOCKED"), "", eNone, eBlink, eNone, 1500, True, "vo_ball3locked" : pupevent 816
                light007.State = 2:light006.State = 2:light070.State = 2
                LightEffect 1
                light058.State = 2
                bLockActive(CurrentPlayer) = False
                bLockActive(CurrentPlayer) = False
                DMD CL(0, "CONCERT"), CL(1, "MULTIBALL IS READY"), "", eNone, eBlink, eNone, 5000, True, ""
...
Sub StartWitchMultiball
    ...
    ' flash show
    DMD CL(0, "CONCERT"), CL(1, "MULTIBALL"), "", eNone, eBlink, eNone, 1500, True, "vo_multiball3"
    Exit_WitchNest
    post001.IsDropped = 0:PlaySoundAt "fx_SolenoidOn", Trigger010
    post002.IsDropped = 0
...
Sub AwardSPJackpot
    If bSPJActivated Then
        ...
        DMD CL(0, FormatScore(tmp)), "SUP FULLSHOW JACKPOT", "", eNone, eBlinkFast, eNone, 1500, True, "vo_superjackpot2"
        DOF 126, DOFPulse
        Addscore tmp
        LightEffect 2
        StopSuperPyramid
...
Sub EnableFinisterra
    DMD "HIT THE TIME STARTS", "BACKSTAGE MULTIBALL", "", eBlink, eBlink, eNone, 1500, True, ""
...
Sub StartFinisterra
    StopBRUJALetter
    DMD CL(0, "BACKSTAGE"), "     MULTIBALL", "", eNone, eBlink, eNone, 1500, True, "vo_multiball" : pupevent 823
    light045.State = 0
    light024.State = 0
    light025.State = 0
    light026.State = 0
...
Sub AwardFinisterraSJP
    ...
    DMD CL(0, FormatScore(tmp)), "BACKSTAGE JACKPOT", "", eNone, eBlinkFast, eNone, 1500, True, "vo_superjackpot"
    bFinisterraSJPActivated = False
    light045.State = 0
    light024.State = 0
    light025.State = 0
...
Sub AwardTombTreasure
    ...
    Select Case TombTreasureCount
        Case 0:
            DMD CL(0, "TICKET PRICE"), CL(1, "15 MILLION"), "", eNone, eBlink, eNone, 1500, True, ""
            Addscore 15000000
        Case 1:
            DMD CL(0, "TICKET PRICE"), CL(1, "SUPER SLINGSHOTS"), "", eNone, eBlink, eNone, 1500, True, ""
            SlingshotValue = 250000
        Case 2:
            DMD CL(0, "TICKET PRICE"), CL(1, "+15MILL SPJ PYRAMID"), "", eNone, eBlink, eNone, 1500, True, ""
            SuperPyramidValue = SuperPyramidValue + 15000000
            StartFelizNavidadCabrones
        Case 3:
            DMD CL(0, "TICKET PRICE"), CL(1, "SUPER 5X COMBO"), "", eNone, eBlink, eNone, 1500, True, ""
            bSuperCombo = True
        Case 4:
            DMD CL(0, "TICKET PRICE"), CL(1, "EXTRA BALL IS LIT"), "", eNone, eBlink, eNone, 1500, True, ""
            Light047.State = 1
        Case 5:
            DMD CL(0, "TICKET PRICE"), CL(1, "BALL SAVE 60 SEC"), "", eNone, eBlink, eNone, 1500, True, ""
            EnableBallSaver 60
        Case 6:
            DMD CL(0, "TICKET PRICE"), CL(1, "COLLECT 2X BONUS"), "", eNone, eBlink, eNone, 1500, True, ""
            BonusMultiplier(CurrentPlayer) = BonusMultiplier(CurrentPlayer) + 2
        Case 7:
            DMD CL(0, "TICKET PRICE"), CL(1, "SUPER PYRAMID 5X SPJ"), "", eNone, eBlink, eNone, 1500, True, ""
            SuperPyramidMultiplier = SuperPyramidMultiplier + 5
        Case 8:
            DMD CL(0, "TICKET PRICE"), CL(1, "50 MILLION"), "", eNone, eBlink, eNone, 1500, True, ""
            Addscore 50000000
        Case 9:
            DMD CL(0, "TICKET PRICE"), CL(1, "EXTRA BALL IS LIT"), "", eNone, eBlink, eNone, 1500, True, ""
    ...
    Select Case FNCStep
        ...
        Case 5                                                                                                                                                                   'all 7 lights
            If light056.State = 1 AND light053.State = 1 AND light055.State = 1 AND light052.State = 1 AND light054.State = 1 AND light049.State = 1 AND light057.State = 1 Then 'all 7 lights are green
                ...
                DMD "FELIZ NAVIDAD CABRON", CL(1, "COMPLETED"), "", eNone, eBlink, eNone, 1500, True, ""
                Addscore 10000000
                StopFNC
                StartBRUJALetter
            End If
