...
Sub AddScore(points)
    ' Only add points if the table isn't tilted
    If (fpTilted = FALSE) Then
        ' Update the internal score variable
        Score(CurrentPlayer) = Score(CurrentPlayer) + points

        ' --- Extra Ball Logic ---
        ' Using 1st Extra Ball threshold
        If (Score(CurrentPlayer) > 750000) And (extraballonescored = False) Then
            scoreextraball()
            extraballonescored = True
        End If

        ' Using 2nd Extra Ball threshold
        ...
        If (ballindrain = False) And (messageon = False) Then
            ' Update the on-table text element (D1)
            D1.Text = " " & Score(CurrentPlayer)

            ' Update Ball Number Display (D2)
            ' CurrentPlayer index is usually 0-based or 1-based depending on table; 
            ' ballsremaining is checked here:
            Select Case ballsremaining(CurrentPlayer)
                Case 3
                    D2.Text = "Ball 1"
                    UpdateB2S Score(CurrentPlayer), 1
                Case 2
                    D2.Text = "Ball 2"
                    UpdateB2S Score(CurrentPlayer), 2
                Case 1
                    D2.Text = "Ball 3"
                    UpdateB2S Score(CurrentPlayer), 3
            End Select
        End If

        ' --- External Display (UltraDMD/FlexDMD) ---
        ' Standalone often struggles with UltraDMD; it's safer to check if the object exists
        If turnonultradmd > 0 Then 
            On Error Resume Next ' Prevents crash if UltraDMD object is missing
...
' Helper Sub to handle B2S text safely for Standalone
Sub UpdateB2S(val, ball)
    On Error Resume Next
    ' In Standalone, Controller (B2S) might not be initialized. 
    ' This string formatting matches your original logic.
    Dim msg : msg = val & "               BALL " & ball
    DisplayB2SText msg
    On Error Goto 0
End Sub
