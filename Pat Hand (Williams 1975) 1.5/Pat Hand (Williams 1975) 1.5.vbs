' ******************************************************************
'     VPX - version by Klodo81, EMUnderdogs, Pat Hand version 1.5
'
...
'- Mod center image playfield
'- In DT mode image BG from Hauntfreaks
'- Added "Same player shoots again" on Backglass DT
'- Some others things
'
'1.5 EMUnderdogs
'- New playfield (SilverSurfer + HauntFreaks + Teisen)
'- Shadow layer (JCalhoun)
'- Minor sound cleanup, remove orphaned sounds (JCalhoun)
'- 3K lockdown bar (Cliffy)
'- Replace cab wall rails with proper primwall (Cliffy)
'- Replace backbar with correct rear channel bar prim (Cliffy)
'- Swapped in new chimes sounds (JCalhoun)
'- Added flipper shadows (JCalhoun)
'- Cleaned up plastics artwork (JCalhoun)
'- Adjusted curves of plastics walls (JCalhoun)
'- New instruction cards (JCalhoun)
'- Custom desktop backdrop for use with B2S (Cliffy)
'- playfield color adjustment (Teisen)
'- Adjustment of metal colors (Teisen)
'- Added animated rollover buttons (Teisen)
'- Playfield lights (Teisen)
'- Addition of illuminated bumper base prims(Teisen)
'- Lighting adjustment of some plastics (Teisen)
'- Ball with warm tone (Teisen)
'- New target texture (JCalhoun)
'- Change high score intitials sound effects (Cliffy)
'- Fix credits not showing past 9 when using b2s (Hauntfreaks)
'- New lighting behind top-row of targets (Teisen)
'
' ******************************************************************
...
Sub Table1_KeyDown(ByVal Keycode)
    ...
    If Keycode = AddCreditKey Then
        If(Tilted = False) Then
            AddCredits 1
            ' some random pitch (0.25), fade near player (0.5)
            PlaySound "fx_coin", 0, 1, 0, 0.25, 0, 0, 0, 0.5
		End If
    End If
    ...
    If keycode = CenterTiltKey Then Nudge 0, 6:PlaySound "fx_nudge", 0, 1, 0, 0.25:CheckTilt
...
Sub SolLFlipper(Enabled)
    If Enabled Then
        ...
        PlaySoundAt SoundFXDOF ("LeftflipperupH-2dB_20ms", 101, DOFOn, DOFFlippers), LeftFlipper2
        LeftFlipper.RotateToEnd
        LeftFlipper2.RotateToEnd
		LeftFlipperOn = 1
    Else
        PlaySoundAt SoundFXDOF ("LeftflipperdownH", 101, DOFOff, DOFFlippers), LeftFlipper
        PlaySoundAt SoundFXDOF ("LeftflipperdownH_20ms", 101, DOFOff, DOFFlippers), LeftFlipper2
...
Const kChime_10 = "ding10_5dB"		'"fx_williamschime10"
Const kChime_100 = "ding100_5dB"		'"fx_williamschime100"
Const kChime_1000 = "ding1000_5dB"	'"fx_williamschime1000"
    ...
    Select Case Points
        ...
        ' sounds  
			If Points = 100 AND(Score(CurrentPlayer) MOD 1000) \ 100 = 0 Then            ' new reel 1000
                PlaySound SoundFXDOF (kChime_1000, 303, DOFPulse, DOFChimes)
            ElseIf Points = 10 AND(Score(CurrentPlayer) MOD 100) \ 10 = 0 Then           ' new reel 100
                PlaySound SoundFXDOF (kChime_100, 302, DOFPulse, DOFChimes)
			...
			ElseIf Points = 100 Then
                PlaySound SoundFXDOF (kChime_100, 302, DOFPulse, DOFChimes)          
			Else
                PlaySound SoundFXDOF (kChime_10, 301, DOFPulse, DOFChimes)
            End If
        Case 20, 30, 40, 50
...
Sub GameTimer_Timer
	...
	FlipperShadowLeft.RotZ = LeftFlipper.CurrentAngle
	FlipperShadowTopLeft.RotZ = LeftFlipper2.CurrentAngle
	FlipperShadowRight.RotZ = RightFlipper.CurrentAngle
...
Sub CheckCards()
	...
	Case 0 : 
		If LTAce.State = 0 And LTJack.State = 0 And LTQueen.State = 0 And LTKing.State = 0 And LT10.State = 0 Then	
			...
			LightBumper001.State = 1 : GILight001.State = 1 : LightBumper002.State = 1 : GILight002.State = 1
			CardsRun = 1
		End If
	Case 1 : 
		If LTAce.State = 0 And LTJack.State = 0 And LTQueen.State = 0 And LTKing.State = 0 And LT10.State = 0 Then	
    ...
    If len(InitialString) = 3 then
        ...
        For i = 5 to 1 step -1
            If i = 1 or(HSNewHigh> HSScore(i) and HSNewHigh <= HSScore(i - 1) ) then
                ...
                HighScoreTimer.interval = 2000
                PlaySound("fx_knocker")    ' PlaySound("fx_Bong") <- "fx_Bong" sound missing, switched to knocker
                Exit Sub
            ElseIf i <5 then
