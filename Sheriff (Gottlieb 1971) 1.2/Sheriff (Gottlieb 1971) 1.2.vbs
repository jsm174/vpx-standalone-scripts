'*
'*        Gottlieb's Sheriff (1971)
'*        http://www.ipdb.org/machine.cgi?id=2116
'*        
'*        Table build/scripted by Loserman76
'*        Playfield/plastics updated/redrawn by GNance
'*        Original images by Pope
'*        
'*        Teisen_MOD by teisen
'*          - Playfield color correction and touch-ups
'*          - cap bumpers corretion (IPDB)
...
'*          - red pins (IPDB)
'*          - other small touches
'*        1.1	Upscaling & music mod by GauGau
'*        1.2	Fleep sounds and sound code added by JCalhoun
'*        
...
Const HSFileName="Sheriff_71VPX.txt"
Const B2STableName="Sheriff_1971"
...
usePUP   = false      ' enable Pinup Player functions for this table. This setting toggles between music playback and pup-pack playback.
cPuPPack = "Sheriff" ' name of the PuP-Pack / PuPVideos folder within PinUPSystem\PUPVideos for this table
...
Sub TriggerLeft1_Hit()
	If TableTilted=false then
'		PlaySound "sensor"
		RandomSoundRollover
		DOF 121, 1
		If LeftOutlaneLight1.state=1 then
			SetMotor(1000)
			IncreaseBonus
		else
			SetMotor(500)
...
Sub TriggerLeft2_Hit()
	If TableTilted=false then
'		PlaySound "sensor"
		RandomSoundRollover
		DOF 122, 1
		If LeftOutlaneLight2.state=1 then
			SetMotor(1000)
			IncreaseBonus
		else
			SetMotor(500)
...
Sub ResetDrops
	for each obj in AllDropTargets
		obj.IsDropped=0
	next
	If DropTargetCounter > 0 Then DOF 123, 2
	DropTargetCounter=0
end Sub
...
sub delaykgclose_timer
 ...
 sub openg_timer
	...
	primgate.RotY=30+(bgpos*10)
     if bgpos=0 then 
'		playsound "postup"	<- sound missing, substituting
...
'	end if
	PlayBonusSound BonusMultiplier
...
sub BumpersOff
	...
	GILight001.visible=False
	GILight002.visible=False
end sub

sub BumpersOn
	Bumper1Light.visible=true
	Bumper2Light.visible=true
	GILight001.visible=True
	GILight002.visible=True
...
Sub SetupReplayTables
	...
	Replay1Table(2)=37000
	Replay1Table(3)=42000
	Replay1Table(4)=46000
	Replay1Table(5)=50000
	Replay1Table(6)=55000
	Replay1Table(7)=61000
	Replay1Table(8)=65000
	...
	Replay2Table(1)=46000
	Replay2Table(2)=51000
	Replay2Table(3)=56000
	Replay2Table(4)=60000
	Replay2Table(5)=64000
	Replay2Table(6)=69000
	Replay2Table(7)=75000
	Replay2Table(8)=79000
	Replay2Table(9)=86000
	...
	Replay3Table(1)=54000
	Replay3Table(2)=59000
	Replay3Table(3)=64000
	Replay3Table(4)=68000
	Replay3Table(5)=72000
	Replay3Table(6)=77000
	Replay3Table(7)=83000
	Replay3Table(8)=87000
	Replay3Table(9)=94000
	Replay3Table(10)=99000
		...
		Select Case MotorPosition
			...
			Case 1:
				...
				if MotorMode=10 then
					AddScore(10)
				End if
				MotorPosition=0:MotorRunning=0:BumpersOn:ToggleSpecial
		End Select
	End If
...
Const kSoundFadeNearBackglass = -1.0
Const kSoundFadeCenter = 0.0
Const kSoundFadeNearPlayer = 1.0
Const kSoundPanLeft = -1.0
Const kSoundPanCenter = 0.0
Const kSoundPanRight = 1.0

'///////////////////////////////  SOUNDS PARAMETERS  //////////////////////////////
Dim GlobalSoundLevel, CoinSoundLevel, PointBellLevel, GottliebChimeLevel, StartupSoundLevel, StartBallSoundLevel
Dim ClickSoundLevel, MotorRunningSoundLevel, BonusSoundLevel, MotorLeerSoundLevel, RotateThroughPlayersSoundLevel
Dim PlungerReleaseSoundLevel, PlungerPullSoundLevel, NudgeLeftSoundLevel
Dim NudgeRightSoundLevel, NudgeCenterSoundLevel, StartButtonSoundLevel, RollingSoundFactor

CoinSoundLevel = 1														'volume level; range [0, 1]
PointBellLevel = 1														'volume level; range [0, 1]
GottliebChimeLevel = 1													'volume level; range [0, 1]
StartupSoundLevel = 1													'volume level; range [0, 1]
StartBallSoundLevel = 1													'volume level; range [0, 1]
ClickSoundLevel = 1														'volume level; range [0, 1]
MotorRunningSoundLevel = 1												'volume level; range [0, 1]
BonusSoundLevel = 1														'volume level; range [0, 1]
MotorLeerSoundLevel = 1													'volume level; range [0, 1]
RotateThroughPlayersSoundLevel = 1										'volume level; range [0, 1]
...
'/////////////////////////////  KNOCKER SOLENOID  ////////////////////////////
Sub KnockerSolenoid()
	' KnockerPosition not defined
'	PlaySoundAtLevelStatic SoundFX("Knocker_1",DOFKnocker), KnockerSoundLevel, KnockerPosition
	PlaySound SoundFXDOF("Knocker_1",124,DOFPulse,DOFKnocker)
End Sub
...
Sub PlayCoinSound()
	PlaySound ("Coin_In_" & Int(Rnd*3)+1), 0, CoinSoundLevel * VolumeDial, kSoundPanCenter, 0, 0, 0, 0, kSoundFadeNearPlayer
End Sub

'/////////////////////////  ADDITIONAL EM SUBROUTINES  /////////////////////
'	PlaySound NAME, LOOP_COUNT, VOLUME, PAN, 0, 0, 0, 0, FADE

Sub PlayEMPointsSound(doChimes, chimeNum)
	If doChimes=0 Then
		PlayPointBellSound chimeNum
	Else
		PlayGottliebChimeSound chimeNum
	End If
End Sub

Const kPointBellPanValue = 0.5
Const kPointBellFadeValue = 0.5

Sub PlayPointBellSound(bellNum)
	Dim sndName
	Select Case bellNum
		Case 0
			sndName = "SpinACard_1_10_Point_Bell"
		Case 1, 2
			sndName = "SpinACard_100_Point_Bell"
		Case Else
			Exit Sub
	End Select
	PlaySound sndName, 0, PointBellLevel * VolumeDial, kPointBellPanValue, 0, 0, 0, 0, kPointBellPanValue
End Sub

Const kGottliebChimePanValue = 0.5
Const kGottliebChimeFadeValue = 0.5


Sub PlayGottliebChimeSound(chimeNum)
	Dim sndName
	Select Case chimeNum
		Case 0
			sndName = "Gottlieb10Points"
		Case 1
			sndName = "Gottlieb100Points"
		Case 2
			sndName = "Gottlieb1000Points"
		Case Else
			Exit Sub
	End Select
	PlaySound sndName, 0, GottliebChimeLevel * VolumeDial, kGottliebChimePanValue, 0, 0, 0, 0, kGottliebChimePanValue
End Sub

Sub PlayClickSound()
	PlaySound "click", 0, ClickSoundLevel * VolumeDial, kSoundPanCenter, 0, 0, 0, 0, kSoundFadeNearBackglass
End Sub

Sub PlayStartupSound()
	PlaySound "GottliebStartup", 0, StartupSoundLevel * VolumeDial, kSoundPanCenter, 0, 0, 0, 0, kSoundFadeNearBackglass
End Sub

Sub PlayStartBallSound(which)
	Dim sndName
	If which=0 Then
		sndName = "StartBall1"
	Else
	...
	End If
	PlaySound sndName, 0, StartBallSoundLevel * VolumeDial, kSoundPanCenter, 0, 0, 0, 0, kSoundFadeNearBackglass
End Sub

Sub PlayMotorRunningSound()
	PlaySound "MotorRunning", 0, MotorRunningSoundLevel * VolumeDial, kSoundPanCenter, 0, 0, 0, 0, kSoundFadeNearBackglass
End Sub

Sub PlayBonusSound(multiplier)
	If multiplier=1 Then
		Playsound "Score1000", 0, BonusSoundLevel * VolumeDial, kSoundPanCenter, 0, 0, 0, 0, kSoundFadeNearBackglass
	Else
		Playsound "Score2000", 0, BonusSoundLevel * VolumeDial, kSoundPanCenter, 0, 0, 0, 0, kSoundFadeNearBackglass
	End If
End Sub

Sub PlayMotorLeerSound()
	PlaySound "MotorLeer", 0, MotorLeerSoundLevel * VolumeDial, kSoundPanCenter, 0, 0, 0, 0, kSoundFadeNearBackglass
End Sub

Sub PlayRotateThroughPlayersSound()
	PlaySound "RotateThruPlayers", 0, RotateThroughPlayersSoundLevel * VolumeDial, kSoundPanCenter, 0, 0, 0, 0, kSoundFadeNearBackglass
End Sub
...
Const OptionLine4="Special lights "
Const OptionLine5=""
Const OptionLine6=""
Const OptionLine7=""
