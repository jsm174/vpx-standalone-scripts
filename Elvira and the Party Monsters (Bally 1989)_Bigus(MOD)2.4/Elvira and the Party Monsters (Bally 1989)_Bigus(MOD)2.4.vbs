...
Sub Table1_Init
	...
	On Error Resume Next
		...
		.SplashInfoLine = "Elvira and the Party Monsters"
	         .HandleKeyboard = 0
         .ShowTitle = 0
         .ShowDMDOnly = 1
         .ShowFrame = 0
...
Sub sw32_Hit():bsBP.AddBall Me : playsound "popper_ball": End Sub
 'Bumpers
Sub Bumper1_Hit : vpmTimer.PulseSw 35 : BCap1.z = -15 : playsound SoundFX("fx_bumper1",DOFContactors): vpmtimer.AddTimer 150, "BCap1.z = 0'" :End Sub
Sub Bumper2_Hit : vpmTimer.PulseSw 36 : BCap2.z = -15 : playsound SoundFX("fx_bumper1",DOFContactors): vpmtimer.AddTimer 150, "BCap2.z = 0'" :End Sub
Sub Bumper3_Hit : vpmTimer.PulseSw 37 : BCap3.z = -15 : playsound SoundFX("fx_bumper1",DOFContactors): vpmtimer.AddTimer 150, "BCap3.z = 0'" :End Sub
 'Wire Triggers
...
Sub FlexDMD_Init() 'default/startup values
	...
	If IsObject(FlexDMD) Then
		...
		With FlexDMDScene
			...
			If Not IsNumeric(vpmColourised) Then vpmColourised = 0 Else vpmColourised = CLng(vpmColourised)
			If vpmColourised = 0 Then
				vpm33Percent = SafePercent(Controller.Games(cGameName).Settings.Value("dmd_perc33"), 33)
				vpm66Percent = SafePercent(Controller.Games(cGameName).Settings.Value("dmd_perc66"), 66)
				vpmOffPercent = SafePercent(Controller.Games(cGameName).Settings.Value("dmd_perc0"), 0)
			End If
...
Function SafePercent(v, defaultPercent)
	'coerce a settings value to a 0..1 fraction with a fallback default percent (0..100)
	'guards against missing/empty/non-numeric INI values that would cause type mismatch
	If IsNumeric(v) Then
		SafePercent = CDbl(v) / 100
	Else
		SafePercent = CDbl(defaultPercent) / 100
	End If
End Function