...
' Initialize POW names array (will be randomly shuffled). Not Used In SOA Table
'
...
' Initialize POW ranks array. Not used in SOA Table
'
...
If keycode = RightMagnaSave And bGameInPlay And Mayhemvote > 0 Then EnableRMagnet
If keycode = RightMagnaSave And bGameInPlay And Mayhemvote = 0 Then 
DisplayB2SText "     MAYHEM ALREADY DECIDED"
D1.Text = "     MAYHEM ALREADY DECIDED"
PlaySound "Fail"

End If

' If keycode = LeftMagnaSave And Not (bGameInPlay) Then SetupMenuResetHighScores() 'Reset High Scores for testing

    If keycode = PlungerKey Then
	PlaySound "fx_launchball"
	StopAttractMode
    ...
    If bGameInPlay AND NOT Tilted Then
        ...
        If keycode = LeftFlipperKey Then SolLFlipper 1 : LightSeqAttract.StopPlay
        If keycode = RightFlipperKey Then SolRFlipper 1 : LightSeqAttract.StopPlay
...
Sub Spinner1_Spin()
    PlaySound "fx_spinner", 0, 0.6, 0.05, 0
	DOF 137, DOFPulse
    If NOT Tilted Then
...
Sub PlaySong(name)
    If bMusicOn Then
        If Song <> name Then
            ...
            Song = name
            If Song = "mu_end" Then
                PlaySound Song, 0, 0.7  'this last number is the volume, from 0 to 1
            Else
...
If Not(bPlayingFinalMission) And Not (bPlayingExtraBall) Then
    DMDFLush
    DMD "Ride.gif", "", "", 1700
...
Sub AwardJackpot() 'award a normal jackpot, double or triple jackpot
   ...
   ' DMDBlink "", "JACKPOT", Jackpot, 100, 10
    StopSound (SoundName) : SoundName = "jackpot" : PlaySound (SoundName)
    AddScore Jackpot
    GiEffect 1
    LightEffect 2
    FlashEffect 2
...
Sub AwardDoubleJackpot() 'in this table the jackpot is always 1 million + 10% of your score
   ...
   ' DMDBlink "", "DOUBLE JACKPOT", Jackpot, 100, 10
    StopSound (SoundName) : SoundName = "jackpot2" : PlaySound (SoundName)
    AddScore Jackpot
    GiEffect 1
    LightEffect 2
    FlashEffect 2
...
Sub CheckHighscore()
    ...
    If tmp> HighScore(3) Then
        vpmtimer.addtimer 2000, "PlaySound ""youregreat"" '"
        HighScore(3) = tmp
        'enter player's name
        HighScoreEntryInit()
    Else
...
Sub HighScoreEntryInit()
    hsbModeActive = True
    StopSound Song:Song = "": PlaySong "sittinontop"
    hsLetterFlash = 0
...
Sub LeftDrainTrigger_Hit()
If Not (bBallSaverIsActive) Then Playsound "Looks like youre fd"
End Sub
...
Sub Kicker1SolenoidPulse()
    Kicker1.kick 185, 15
    Playsound SoundFXDOF("bumper_retro",108,DOFPulse,DOFContactors)
	DOF 114, DOFPulse
End Sub
...
Sub Kicker2SolenoidPulse()
    Kicker2.kick 215, 15
    Playsound SoundFXDOF("bumper_retro",115,DOFPulse,DOFContactors)
	DOF 114, DOFPulse
End Sub
...
Sub Kicker3SolenoidPulse()
    Kicker3.kick 165, 15
    Playsound SoundFXDOF("bumper_retro",116,DOFPulse,DOFContactors)
	DOF 114, DOFPulse
End Sub
...
Sub Kicker4SolenoidPulse()
    Kicker4.kick 270, 25
    Playsound SoundFXDOF("bumper_retro",117,DOFPulse,DOFContactors)
End Sub
   ...
   AddScore(1000)
	If bPlayingMission Then
		...
		If bPlayingMission3 Then
			...
			playsound SoundFXDOF("Stahl1",106,DOFPulse,DOFContactors)
			CheckJets()
		Else
			playsound SoundFXDOF("fx_bumperleft",106,DOFPulse,DOFContactors)
		End If
   Else
...
Sub Bumper2_Hit()
   ...
   AddScore(1000)
	If bPlayingMission Then
		...
		If bPlayingMission3 Then
			RemainingJets = RemainingJets - 1
			playsound SoundFXDOF("Stahl2",105,DOFPulse,DOFContactors)
			CheckJets()
		Else
...
Sub Bumper3_Hit()
   ...
   AddScore(1000)
	If bPlayingMission Then
		...
		If bPlayingMission3 Then
			RemainingJets = RemainingJets - 1
			playsound SoundFXDOF("Stahl3",107,DOFPulse,DOFContactors)
			CheckJets()
		Else
...
' This subroutine decrements Clay's health during mission 5.
'
' *** DOCAM OPTIMIZED ***
'
Sub ScoreMission5()
...
RideTimer = RideTimer - 10
If RideTimer < 15 Then RideTimer = 15 
...
Sub PlaySlugSound()
	...
	Select Case SlugSound
		Case 1: Playsound "DropTargetHit"
		Case 2: Playsound "DropTargetHit"
		Case 3: Playsound "DropTargetHit"
		Case 4: Playsound "DropTargetHit"
		Case 5: Playsound "DropTargetHit"
		Case 6: 
