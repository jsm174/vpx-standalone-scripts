...
'*        http://www.ipdb.org/machine.cgi?id=2002
'*        
'*
'*
...
Const HSFileName="RollerCoaster_71VPX.txt"
Const B2STableName="RollerCoaster_1971"
...
Dim SpinPos,Spin,Count,Count1,Count2,Count3,Reset,VelX,VelY,BallSpeed,LitSpinner,SpinArrow
...
'	GameOverReel.SetValue(1)
	...
	If B2SOn Then
		...
		Controller.B2SSetData 81,1 'Chr
...
Sub Table1_KeyDown(ByVal keycode)
   ...
   if keycode =StartGameKey and Credits>0 and InProgress=true and Players>0 and Players<2 and BallInPlay<2 then
		Credits=Credits-1
		If Credits < 1 Then DOF 117, 0
		CreditsReel.SetValue(Credits)
		Players=Players+1
		CanPlayReel.SetValue(Players)
		playsound "AddPlayer EM"
		If B2SOn Then
...
Sub Target1_hit
	...
	If TableTilted=false then
		...
		LightsStatus(1)=0
		SetMotor(50)
		RolloverLight1.state=0
		TargetLight1.state=0
		LowerRightTargetLight.state=1
		
		If BallsPerGame=3 then
			LightsStatus(4)=0
			RolloverLight4.state=0
...
Sub Target2_hit
	...
	If TableTilted=false then
		...
		LightsStatus(2)=0
		SetMotor(50)
		RolloverLight2.state=0
		TargetLight2.state=0
		LowerLeftTargetLight.state=1
		If BallsPerGame=3 then
			LightsStatus(3)=0
			RolloverLight3.state=0
...
Sub Spinner_Hit(Idx)
	...
	BallSpeed=Int(Abs(activeball.VelX*10))+(Abs(activeball.VelY*10)) 'was 5
...
Sub ScoreSpinner
	if SpinnerLight.state=1 then
		if Spin(SpinPos)=50 then
			...
			queuedscore=queuedscore+Spin(SpinPos)
		end if
	else
		queuedscore=queuedscore+Spin(SpinPos)
	end if
...
Sub NumberSequenceRelay_timer
	if MotorRunning=1 or InProgress=false then
		exit sub
	end if
	if RolloverLight1.state=0 and RolloverLight2.state=0 and RolloverLight3.state=0 and RolloverLight4.state=0 then
...
Sub BED_timer()
	...
	Playsound"Right_KickerHoleKick"
	DOF 115, 2
...
sub savehs
	...
	Set ScoreFile=FileObj.CreateTextFile(UserDirectory & HSFileName,True)
		...
		next
'*****************************************hier werden die Spielergebnisse abgespeichert
		for xx=1 to 4 'GABY
			scorefile.writeline Score(xx)
		next
'*****************************************
		ScoreFile.Close
...
'**********************************hier werden die Spielergebnisse zurückgeladen
...
'	    HighScore=cdbl(temp1)
		...
		if HighScore<1 then
			...
			HSName(4) = temp14
			HSName(5) = temp15
'****************************** hier werden die zurückgeladenen Spielergebnisse zugewiesen
		Score(1) = cdbl (temp16)
		Score(2) = cdbl (temp17)
'************************************
		end if
		LDB.enabled=1 '****** hier werden die zugewiesenen Scores and das BG gesandt 
		Set ScoreFile=Nothing
	    Set FileObj=Nothing		
end sub

Sub LDB_Timer()			'T=1000ms
		If B2SOn Then
		For x = 1 to 2
			Controller.B2SSetScorePlayer x, Score(x)
		Next
		End If
		LDB.enabled=0
End sub
...
Sub LightsOn
	if LightsStatus(1)=1 then
		RolloverLight1.state=1
	...
	if LightsStatus(2)=1 then
		RolloverLight2.state=1
	...
	if LightsStatus(3)=1 then
		RolloverLight3.state=1
	...
	if LightsStatus(4)=1 then
		RolloverLight4.state=1
...
Sub SetupReplayTables
	...
	Replay1Table(8)=3600
	Replay1Table(9)=3900
	Replay1Table(10)=4200
	Replay1Table(11)=4400
	Replay1Table(12)=4900
	...
	Replay2Table(8)=5000
	Replay2Table(9)=5300
	Replay2Table(10)=5600
	Replay2Table(11)=5800
	Replay2Table(12)=6300
	...
	Replay3Table(8)=5800
	Replay3Table(9)=6100
	Replay3Table(10)=6400
	Replay3Table(11)=6600
	Replay3Table(12)=7100
	...
	If MotorPosition > 0 Then
		Select Case MotorPosition
			...
			Case 1:
				...
				End if
				MotorPosition=0:MotorRunning=0:BumpersOn:LightsOn
		End Select
	End If
End Sub
...
Sub Metalplates_Hit (idx)
	PlaySound "Grids", 0, OV, AudioPan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, AudioFade(ActiveBall)
End Sub
...
Sub Gates_Hit (idx)
	PlaySound "fx_gate", 0, OV, AudioPan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, AudioFade(ActiveBall)
End Sub
...
Sub Posts_Hit(idx)
 	...
 	If finalspeed > 5 then 
		PlaySound "fx_rubber2", 0, OV, AudioPan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, AudioFade(ActiveBall)
	End if
...
Sub RandomSoundRubber()
	Select Case Int(Rnd*3)+1
		Case 1 : PlaySound "rubber_hit_1", 0, OV, AudioPan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, AudioFade(ActiveBall)
		Case 2 : PlaySound "rubber_hit_2", 0, OV, AudioPan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, AudioFade(ActiveBall)
		Case 3 : PlaySound "rubber_hit_3", 0, OV, AudioPan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, AudioFade(ActiveBall)
	End Select
End Sub
...
Const OptionLine4="Reset Number sequence"
Const OptionLine5=""
Const OptionLine6=""
Const OptionLine7=""
	...
	For linecounter = 1 to MaxOption
		...
		Select Case linecounter
			...
			Case 4:
				...
				if SpecialLightOption=1 then
					tempstring = "At game start only"
				else
					tempstring = "Each new ball"
				end if
