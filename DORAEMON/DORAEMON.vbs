...
Const cGameName = "DORAEMON"
Const TableName = "DORAEMON"
...
Sub ChangeSong
    ...
    tmp = INT(RND * 3)
    Select Case tmp
        Case 0:PlaySong "m_main2"
        Case 1:PlaySong "m_main2"
        Case 2:PlaySong "m_main2"
    End Select
end if
End Sub
...
Sub EndOfBall()
	...
	If(Tilted = False) Then
        ...
        DMD CL(0, FormatScore(AwardPoints)), CL(1, "TARGET BONUS: " & TargetBonus), "_", eBlink, eNone, eNone, 300, False, "po_bonus8"

        AwardPoints = DoraBonus * 50000
        TotalBonus = TotalBonus + AwardPoints
        DMD CL(0, FormatScore(AwardPoints) ), CL(1, "DORA BONUS: " & DoraBonus), "_", eBlink, eNone, eNone, 300, False, "po_bonus8"

        AwardPoints = EmonBonus * 50000
        TotalBonus = TotalBonus + AwardPoints
        DMD CL(0, FormatScore(AwardPoints) ), CL(1, "EMON BONUS: " & EmonBonus), "_", eBlink, eNone, eNone, 300, False, "po_bonus8"

        AwardPoints = DoraEmonBonus * 100000
        TotalBonus = TotalBonus + AwardPoints
        DMD CL(0, FormatScore(AwardPoints) ), CL(1, "DORAEMON BONUS: " & DoraEmonBonus), "_", eBlink, eNone, eNone, 300, False, "po_bonus8"

        AwardPoints = CatBonus * 100000
        TotalBonus = TotalBonus + AwardPoints
        DMD CL(0, FormatScore(AwardPoints) ), CL(1, "CAT BONUS: " & CatBonus), "_", eBlink, eNone, eNone, 300, False, "po_bonus8"

        AwardPoints = HomeBonus * 50000
        TotalBonus = TotalBonus + AwardPoints
        DMD CL(0, FormatScore(AwardPoints) ), CL(1, "HOME BONUS: " & HomeBonus), "_", eBlink, eNone, eNone, 300, False, "po_bonus8"

        AwardPoints = AnywhereBonus * 50000
        TotalBonus = TotalBonus + AwardPoints
        DMD CL(0, FormatScore(AwardPoints) ), CL(1, "DORADOOR BONUS: " & AnywhereBonus), "_", eBlink, eNone, eNone, 300, False, "po_bonus8"

        AwardPoints = RampBonus1 * 25000
        TotalBonus = TotalBonus + AwardPoints
        DMD CL(0, FormatScore(AwardPoints) ), CL(1, "RAMP BONUS 1: " & RampBonus1), "_", eBlink, eNone, eNone, 300, False, "po_bonus8"

        AwardPoints = RampBonus2 * 25000
        TotalBonus = TotalBonus + AwardPoints
        DMD CL(0, FormatScore(AwardPoints) ), CL(1, "RAMP BONUS 2: " & RampBonus2), "_", eBlink, eNone, eNone, 300, False, "po_bonus8"

        AwardPoints = RampBonus3 * 25000
        TotalBonus = TotalBonus + AwardPoints
        DMD CL(0, FormatScore(AwardPoints) ), CL(1, "RAMP BONUS 3: " & RampBonus3), "_", eBlink, eNone, eNone, 300, False, "po_bonus8"

        AwardPoints = ALLRampBonus * 75000
        TotalBonus = TotalBonus + AwardPoints
        DMD CL(0, FormatScore(AwardPoints) ), CL(1, "ALL RAMPS BONUS: " & ALLRampBonus), "_", eBlink, eNone, eNone, 300, False, "po_bonus8"

        AwardPoints = MulitballBonus * 150000
        TotalBonus = TotalBonus + AwardPoints
        DMD CL(0, FormatScore(AwardPoints) ), CL(1, "MULTIBALL BONUS: " & MulitballBonus), "_", eBlink, eNone, eNone, 300, False, "po_bonus8"

        AwardPoints = BumperBonus * 100000
        TotalBonus = TotalBonus + AwardPoints
        DMD CL(0, FormatScore(AwardPoints) ), CL(1, "BUMPER BONUS: " & BumperBonus), "_", eBlink, eNone, eNone, 300, False, "po_bonus8"
        
		DMD CL(0, FormatScore(TotalBonus) ), CL(1, "TOTAL BONUS " & " XII" & BonusMultiplier(CurrentPlayer) ), "_", eBlinkFast, eNone, eNone, 1000, True, "po_bonus7"
        TotalBonus = TotalBonus * BonusMultiplier(CurrentPlayer)
        
		addScore TotalBonus

		' add a bit of a delay to allow for the bonus points to be shown & added up
		vpmtimer.addtimer 6100, "EndOfBall2 '"
...
Dim HoleBonus, BumperBonus, ALLRampBonus, RampBonus1, RampBonus2, RampBonus3, MulitballBonus, TargetBonus, DoraBonus, EmonBonus, DoraEmonBonus, CatBonus, HomeBonus, AnywhereBonus    
...
Sub CheckbBank
    ...
    DMD "_", CL(0, "BANK BONUS"), "_", eNone, eBlink, eNone, 3000, True, "po_Bonus3"
	AddScore 50000
	li009.State=0
	li010.State=0
	li011.State=0
...
Sub CheckbNobi
    ...
    DMD "_", CL(0, "NOBI BONUS"), "_", eNone, eBlink, eNone, 3000, True, "AMAZING"
	AddScore 50000
	LeftInlane.State=0
	LeftOutlane.State=0
	RightInlane.State=0
...
If li005.State = 0 then
PlayQuote
li005.State = 1
DMD "_", CL(0, "RAMP 1 LOCKED"), "_", eNone, eBlink, eNone, 500, True, ""
elseif li005.State = 1 then
PlayQuote
end if
Addscore 5000
CheckbRamp
...
PlayQuote
li006.State = 1
DMD "_", CL(0, "RAMP 2 LOCKED"), "_", eNone, eBlink, eNone, 500, True, ""
elseif li006.State = 1 then
PlayQuote
end if
Addscore 5000
CheckbRamp
...
Sub TBRP3_UnHit()				'There is a trigger on the ramp (after the top) so when the ball rolls of it, it must get stuck behind the wall on the ramp
    If li001.State=1 then 
	AddScore 5000
	end if
	If li002.State=1 then 
...
If li007.State = 0 then
PlayQuote
li007.State = 1
DMD "_", CL(0, "RAMP 2 LOCKED"), "_", eNone, eBlink, eNone, 500, True, ""
elseif li007.State = 1 then
PlayQuote
end if
Addscore 5000
CheckbRamp
...
Sub CheckbRamp
    If(li005.State = 1) And(li006.State = 1) And(li007.State = 1) Then
    DMD "_", CL(0, "RAMP BONUS"), "_", eNone, eBlink, eNone, 3000, True, "po_Bonus3"
	AddScore 50000
	li017.State=1
	li005.State=0
	li006.State=0
...
Sub TRL1_Hit
li024.state = 2
vpmtimer.addtimer 3000, "li024.state = 1 ' "
End Sub
Sub TRL2_Hit
li008.state = 2
vpmtimer.addtimer 3000, "li008.state = 1 ' "
End Sub
Sub TRL3_Hit
li021.state = 2
FlashForMs Flasher6, 1000, 50, 0
vpmtimer.addtimer 3000, "li021.state = 1 ' "
End Sub

Sub TRL4_Hit
li022.state = 2
FlashForMs Flasher7, 1000, 50, 0
vpmtimer.addtimer 3000, "li022.state = 1 ' "
End Sub

Sub TRL5_Hit
li023.state = 2
vpmtimer.addtimer 3000, "li023.state = 1 ' "
End Sub
...
Dim DoraAShake, DoraBShake

Sub DoraAShaker()
    DoraAShake = 6
    DoraATimer.Enabled = True
End Sub

Sub DoraATimer_Timer()
    DoraA.Transz = DoraAShake / 2
    If DoraAShake = 0 Then Me.Enabled = False:Exit Sub
    If DoraAShake < 0 Then
        DoraAShake = ABS(DoraAShake) - 0.1
    Else
        DoraAShake = - DoraAShake + 0.1
    End If
End Sub

Sub DoraBShaker()
    DoraBShake = 6
    DoraBTimer.Enabled = True
End Sub

Sub DoraBTimer_Timer()
    DoraB.Transz = DoraBShake / 2
    If DoraBShake = 0 Then Me.Enabled = False:Exit Sub
    If DoraBShake < 0 Then
        DoraBShake = ABS(DoraBShake) - 0.1
    Else
        DoraBShake = - DoraBShake + 0.1
    End If
End Sub
...
Sub CheckbC19
    If(li012.State = 1) And(li013.State = 1) And(li014.State = 1) Then
	DMD "_", CL(0, "CAT BONUS"), "_", eNone, eBlink, eNone, 3000, True, ""
	Playsound "CatBonus2"
    AddScore 25000
	li015.State=1
	li012.state=0
...
FlashForMs Flasher2, 1000, 50, 0
    ...
    DMDFlush
    DMD CL(0, FormatScore(Score(Currentplayer) ) ), CL(1, bumperHits& " BUMPER HITS LEFT"), "_", eNone, eNone, eNone, 300, True, ""
    CheckBumpers
End Sub
...
Sub Bumper3_Hit() ' what happens when the bumper is hit is inside the sub
	If li001.State=1 then 
	AddScore 500
	end if
	If li002.State=1 then 
...
Sub T1_Hit()
If Tilted Then Exit Sub
	If li001.State=1 and li019.State=0 then
	li019.State=1
	AddScore 2000
	PlaySound "Target1"
	...
	If li001.State=1 and li019.State=1 then
	AddScore 2000
	PlaySound "Target1"
	TargetBonus = TargetBonus + 1
	Checkb3x
...
Sub T2_Hit()
If Tilted Then Exit Sub
	If li001.State=1 and li020.State=0 then
	li020.State=1
	AddScore 2000
	PlaySound "Target1"
	TargetBonus = TargetBonus + 1
	...
	If li001.State=1 and li020.State=1 then
	AddScore 2000
	PlaySound "Target1"
	TargetBonus = TargetBonus + 1
	Checkb3x
...
Sub CheckbDora
	If li003.State = 1 Then Exit Sub
    If(li025.State = 1) And(li026.State = 1) And(li027.State = 1) And(li028.State = 1) Then
		DMD "_", CL(0, "DORA BONUS"), "_", eNone, eBlink, eNone, 3000, True, "po_Bonus5"
        AddScore 25000
        li003.State = 1
    End If
End Sub
...
Sub CheckbEmon
    if li004.State = 1 Then Exit Sub
	if(li029.State = 1) And(li030.State = 1) And(li031.State = 1) And(li032.State = 1) Then
		DMD "_", CL(0, "EMON BONUS"), "_", eNone, eBlink, eNone, 3000, True, "po_Bonus5"
        AddScore 25000
        li004.State = 1
    End If
End Sub
...
Sub CheckbDoraEmon
    If(li025.State = 1) And(li026.State = 1) And(li027.State = 1) And(li028.State = 1) And(li029.State = 1) And(li030.State = 1) And(li031.State = 1) And(li032.State = 1) Then
    DMD "_", CL(0, "DORAEMON BONUS"), "_", eNone, eBlink, eNone, 3000, True, "doreamonn"
	AddScore 50000
	li033.State=1
	li025.State=0
	li026.State=0
...
Sub Checkb2x
    If(li019.State = 1) Or(li020.State = 1) Then
    DMD "_", CL(0, "BONUS 2X"), "_", eNone, eBlink, eNone, 2000, True, "po_Bonus1"
	li001.State=1
	vpmtimer.addtimer 60000, "li001.state = 0 ' "
	vpmtimer.addtimer 60000, "li019.State = 0 ' "
	vpmtimer.addtimer 60000, "li020.State = 0 ' "
	End If
End Sub

Sub Checkb3x
    If(li019.State = 1) And(li020.State = 1) Then
    DMD "_", CL(0, "BONUS 3X"), "_", eNone, eBlink, eNone, 2000, True, "po_Bonus2"
	li002.State=1
	li001.State=0
...
li016.state = 1
DMD "_", CL(0, "NOBITA HOME BONUS"), "_", eNone, eBlink, eNone, 3250, True, ""
PlaySound "home"
addScore 5000
vpmtimer.addtimer 3250,  "kickouty2 ' "
End Sub
...
addScore 1000
vpmtimer.addtimer 1000,  "kickouty ' "
End Sub

sub kickouty
Kick_HOME1.Kick 180,14
PlaySound "fx_popper"
end sub

sub kickouty2
Kick_HOME.Kick 180,14
PlaySound "fx_popper"
end sub
...
SlotAward = Array("a-smallpoints", "a-copterpoints", "a-bigpoints", "a-smallpoints", "a-multiball", "a-30sec", "a-smallpoints", "a-doorpoints", "a-bigpoints", "a-logopoints")
...
Sub GiveSlotAward()
    ...
    DMDFlush
    tmp = INT(RND * 10)
    DMD "", "", SlotAward(tmp),  eNone, eNone, eBlinkFast, 1000, True, "po_knock"

    Select Case tmp
        Case 0:AddScore INT(10000 * RND * 9)  'small points
		Case 1:AddScore 50000				   'copterpoints
        Case 2:AddScore INT(100000 * RND * 9) 'big points
		Case 3:AddScore INT(10000 * RND * 9)  'small points
		Case 4:li035.state = 1
        Case 5:EnableBallSaver 30             'ball saver 30 seconds
        Case 6:AddScore INT(10000 * RND * 9)  'small points
        Case 7:AddScore 100000  'doorpoints
        Case 8:AddScore INT(100000 * RND * 9) 'big points
		'Case 9:AddScore INT(1000 * RND * 9)  'small points
		Case 9:Addscore 500000'logopoints
    End Select
	GiEffect 1
